...
% It is also possible to create the class object by inputting the
...
% (improved Siddon is always used) and PSF blurring is not available (can
% be used manually).
% Compute the (full) CT detector coordinates
...
% coordinates.
% NOTE: The two zeros correspond to the (horizontal) source offset and bed
% offset (multiple bed positions), respectively.
...
raw_SinM = 1 ./ raw_SinM;
options.subsets = 16;
% Subset based class object can be created also by the other two methods
% presented above
A = forwardBackwardProjectCT(options);
% [x,y,z] = CTDetectorCoordinatesFull(-options.angles,options.sourceToDetector,options.sourceToCRot,options.dPitch,options.xSize,...
%     options.ySize);
...
% tau = 1/powerMethod(A, 50, true);
tau = 0.030329432584542;
alpha = 0.01;
Niter = 500;
sigma = tau;
sigma2 = sigma;
...
%% Matrix example using MLEM

options.subsets = 1;
% For matrix version, implementation 1 must be used
options.implementation = 1;
% Again, the other two class constructor methods are valid as well
...
for iter = 1 : 50
    for osa_iter = 1 : options.subsets
        ...
        % It is recommended to clear the system matrix before creating a
        % new one when using subsets
%         clear B
    end
end
ff = reshape(f, options.Nx,options.Ny,options.Nz);
