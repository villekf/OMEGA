...
OMEGA is a MATLAB toolkit to reconstruct data obtained with a positron emission tomography device. This toolkit also allows to easily reconstruct ASCII, LMF or Root  data obtained from GATE simulations. See Features section below for more information on available features and Known Issues and Limitations for toolkit limitations.
...
For more information see [how_to_use.txt](how_to_use.txt).
...
This toolkit also uses ArrayFire library for the GPU/OpenCL implementation. You can find AF binaries from here:  
