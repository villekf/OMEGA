...
% Use CT-images (DICOM-images) for the attenuation. If set to false, will
% use the below attenuation data file. When true, You will be prompted for
% the FOLDER containing the (DIMOCM) CT-images.
...
options.tube_width_z = 0;%options.cr_pz;
...
if options.only_reconstructions == false || options.precompute_all && ~options.only_reconstructions
    if options.use_machine == 0
        ...
        if options.use_machine == 1
            ...
            options.coincidences = load_data_mCT(options);
        else
            % Sinogram data from 32-bit listmode or (uncompressed) sinogram data
...
%%% applied to the sinogram. Using this prevents the need to completely
%%% compute the sinogram again. The uncorrected sinogram remains untouched.
