%% MATLAB codes for GATE PET reconstruction using ASCII, ROOT or LMF output with a custom gradient-based prior
...
options.beta_weighted_mean_OSL_OSEM = 0.1;
...
options.beta_custom_OSL_OSEM = 0.1;
% Regularization parameter for custom prior with OSL-MLEM
options.beta_custom_OSL_MLEM = 1.5;
...
options.beta_custom_ROSEM_MAP = 2;
% Regularization parameter for custom prior with RBI
options.beta_custom_OSL_RBI = 0.1;
% Regularization parameter for custom prior with OSL-(A)COSEM
options.beta_custom_OSL_COSEM = 1;
%%% Regularization parameter for custom prior with PKMA
options.beta_custom_PKMA = 0.1;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Necessary prepass phase for reconstruction

[options, pz] = custom_prior_prepass(options);

%% Custom prior reconstruction
...
for t = 1 : options.partitions
    ...
    for iter = 1 : options.Niter
        ...
        for osa_iter = 1 : options.subsets
            ...
            %%% OSL-OSEM
%             options.grad_OSL_OSEM = custom_prior(options.im_vectors.custom_OSL_apu);
            options.grad_OSL_OSEM = MRP(options.im_vectors.custom_OSL_apu, options.medx, options.medy, options.medz, options.Nx, options.Ny, options.Nz, options.epps, options.tr_offsets, options.med_no_norm);
            %%% OSL-MLEM (implementation 2 only)
            % options.grad_OSL_MLEM = custom_prior(options.im_vectors.custom_MLEM_apu);
            %%% MBSREM
