...
%%% Use Vision data
% 0/false = Don't use Vision machine data (use GATE data instead)
...
% use the below attenuation data file. You will be prompted for the FOLDER
% containing the CT-images.
...
% is thrown.
if ispc % Windows
    options.fpath = 'I:\Väikkäri\MAT-tiedostot\PET\KYS';
else % Unix/MAC
    options.fpath = '/wrk/GATE/output/KYS';
end
...
if options.precompute && options.only_reconstructions == false && options.precompute_all == false && ~options.only_sinos || options.precompute_all && options.precompute
...
if ~options.only_reconstructions && ~options.only_sinos || options.precompute_all && ~options.only_reconstructions
    if options.use_machine == 0
        ...
        if options.use_machine == 1
            % List-mode data
            options.coincidences = load_data_Vision(options);
        else
