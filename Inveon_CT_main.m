...
%%% Axial FOV (mm)
options.axial_fov = 132.7816/(3 / numel(options.use_N_positions));
...
options.Nz = 582/(3 / numel(options.use_N_positions));
...
%     options.dPitch, options.sourceToDetector, options.sourceToCRot, options.horizontalOffset, options.bedOffset, options.subsets, ...
%     options.implementation, options.use_device);

f = options.x0(:);
raw_SinM = double(options.SinM(:));
for iter = 1 : 5
...
f = options.x0(:);
% raw_SinM = double(options.SinM(:));
raw_SinM = double(options.SinM(A.index));
for iter = 1 : 1
...
for iter = 1 : 2
    for osa_iter = 1 : options.subsets
        ...
        x = B * (y ./ raw_SinM(A.nn(osa_iter) + 1:A.nn(osa_iter+1)));
        if options.use_psf
