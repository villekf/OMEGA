%% MATLAB codes for GATE PET reconstruction using ASCII, LMF and ROOT output
...
%%% Columns corresponding to the R-sectors (or blocks) (see GATE wiki on
% ASCII file format)
% Corresponds to (from GATE Wiki): Column 12 to 17 : volume IDs* (first
% single)
options.rsector_ind1 = (8);
% (second single)
options.rsector_ind2 = (24);
%%% Module indices (columns)
% if no modules present use 0 (i.e. ECAT geometry)
% (first single)
options.module_ind1 = (9);
% (second single)
options.module_ind2 = (25);
...
%%% index of the source location of the first single (X-dimension, first
% column of the three)
% Corresponds to (from GATE Wiki): Column 4 to 6 : XYZ position of the
% source in world referential (first single)
% Applicable only if options.source = true
options.source_index1 = 2;
%%% index of the source location of the second single (X-dimension, first
% column of the three)
% Corresponds to (from GATE Wiki): Column 27 to 29 : XYZ position of the
% source in world referential (second single)
% Applicable only if options.source = true
options.source_index2 = 18;
...
% true
% Applicable only if randoms, scatter or trues are extracted
...
% true
% Applicable only if randoms, scatter or trues are extracted
options.event_index2 = 17;
%%% Index for number of Compton interaction in phantom/detector, first single
% Corresponds to (from GATE Wiki): Column 18 : Number of Compton
% interactions in phantoms before reaching the detector (first single)
...
% Applicable only if catter is extracted
options.scatter_index1 = 13;
%%% Index for number of Compton interaction in phantom, second single
% Corresponds to (from GATE Wiki): Column 41 : Number of Compton
% interactions in phantoms before reaching the detector (second single)
% This is only applicable if store_scatter = true
% Same rules apply as above
% Applicable only if catter is extracted
options.scatter_index2 = 29;
...
%%% Form only sinograms or raw data matrix (no reconstructions)
...
% (OSEM & MLEM only)
options.reconstruction_method = 4;
...
if options.only_reconstructions == false && options.only_sinos == false || options.precompute_all && ~options.only_reconstructions
    options.coincidences = load_data(options);
end
 
%% Form the sinograms
 
if options.only_reconstructions == false && options.use_raw_data == false || options.precompute_all && ~options.only_reconstructions
