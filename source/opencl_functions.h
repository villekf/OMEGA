...
inline void orth_distance_bpfp(const int tempi, const uint Nx, const float y_diff, const float x_diff, const float y_center,
	__constant float* x_center, const float kerroin, const float length_, float* temp, const uint temp_ijk, 
	const uint d_Ny, const uint d_N, const int tempj, const uchar fp, float* d_output_local, const __global float* d_rhs, __global float* d_Summ, 
	__global float* d_output, const uchar no_norm, float d_rhs_local, const bool FP_bool) {
...
// Denominator, forward/back projection, orthogonal distance based ray tracer
inline void orth_distance_bpfp_3D(const int tempi, const uint Nx, const uint Nz, const float y_diff, const float x_diff, const float z_diff,
	const float y_center, __constant float* x_center, __constant float* z_center, const float crystal_size_z, float* temp, const uint temp_ijk, 
	const uint d_Ny, const uint d_N, const int tempk, const uint Nxy, const float xs, const float ys, const float zs, const int dec, const uchar fp, float* d_output_local, const __global float* d_rhs, __global float* d_Summ,
	__global float* d_output, const uchar no_norm, float d_rhs_local, const bool FP_bool) {
...
// Denominator (forward projection), orthogonal distance based ray tracer, multi-GPU
inline void orth_distance_perpendicular_bpfp(const float diff1, __constant float* center1, const float kerroin,
	...
	const __global float* d_rhs, const uchar fp, float* d_output_local, __global float* d_output, __global float* Summ, const uchar no_norm, const float d_rhs_local, const bool FP_bool) {
...
// Denominator (forward projection), orthogonal distance based ray tracer, multi-GPU
inline void orth_distance_perpendicular_bpfp_3D(__constant float* center1, const float center2, __constant float* z_center,
	...
	__global float* d_output, __global float* Summ, const uchar no_norm, const float d_rhs_local, const bool FP_bool) {
...
inline void orth_distance_perpendicular_multi_3D(__constant float* center1, const float center2, __constant float* z_center,
	...
	const float xs, const float ys, const float zs, const float xl, const float yl, const float zl, const float crystal_size_z, const uint Nyx, const uint Nz,
	const uchar no_norm, __global float* Summ, __global float* d_rhs_OSEM, const bool FP, const bool RHS) {
