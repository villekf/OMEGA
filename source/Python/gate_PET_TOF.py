...
options.normalization_options = np.array([True, True, True, True])
...
# empty array or inf.
options.normalization_phantom_radius = np.inf
...
options.normalization_attenuation = None
...
# options.maskFP = np.ones((options.nRowsD,options.nColsD),dtype=np.bool_)
# Backprojection mask
# If nonempty, the mask will be applied. If empty, or completely omitted, no
# mask will be considered.
...
#     else:
#         options.SinM = Sino
#     np.save(options.machine_name + options.name +  ' _TOFsinograms_combined_static_' + str(options.Ndist) + str(options.Nang) + '_' + str(options.NSinos) + '_span' + str(options.span) + '_' + str(options.TOF_bins) + 'bins_' 
#             + str(options.TOF_width*1e12) + 'psBinSize_' + str(options.TOF_noise_FWHM*1e12) + 'psFWHM', Sino)
