...
def increaseSampling(options, x=None, y=None, interpolate_sinogram=False):
    ...
    Y = np.asfortranarray(np.arange(1, options.Ndist * options.sampling + 1, options.sampling))
    X = np.asfortranarray(np.arange(1, options.Nang + 1))
    X_grid, Y_grid = np.meshgrid(X, Y)
    
    Yq = np.asfortranarray(np.arange(1, options.Ndist * options.sampling + 1))
    Xq = np.asfortranarray(np.arange(1, options.Nang + 1))
    Xq_grid, Yq_grid = np.meshgrid(Xq, Yq)
    
    # Interpolation
    def interp_surface(data):
        from scipy.interpolate import interp2d
        f = interp2d(X, Y, data.astype(np.float64), kind=options.sampling_interpolation_method, bounds_error=False, fill_value=0.0)
        return f(Xq, Yq)
    ...
    # Interpolate sinogram if needed
...
def interpolateSinog(SinM, sampling, Ndist, Nang, sampling_interpolation_method):
    ...
    X, Y = np.asfortranarray(np.meshgrid(np.arange(1, Nang + 1), np.arange(1, Ndist * sampling + 2, sampling)))
    Xq, Yq = np.asfortranarray(np.meshgrid(np.arange(1, Nang + 1), np.arange(1, Ndist * sampling + 2)))
    ...
    for uu in range(SinM.shape[3]):
        for kk in range(SinM.shape[2]):
            ...
            f_interp = interp2d(X[0, :], Y[:, 0], sinogram_slice, kind=sampling_interpolation_method)
            interpolated = f_interp(Xq[0, :], Yq[:, 0])
            SinM_uus[:, :, kk, uu] = interpolated[:-1, :]

    return SinM_uus.astype(np.float32).reshape(-1)