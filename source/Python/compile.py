...
def CommandLine(args=None):
    ...
    if os.name == 'nt':
        ...
        if len(afpath) > 0:
            ...
            aflib = os.environ['AF_PATH'] + "/lib"
        elif os.path.exists('/opt/arrayfire/'):
            afpath = '/opt/arrayfire/include'
            aflib = '/opt/arrayfire/lib'

        metalpath = ''
        if 'METALCPP_INCLUDE_PATH' in os.environ:
            metalpath = os.environ['METALCPP_INCLUDE_PATH']
        ...
        try:
            result = subprocess.run([
                ...
                '-framework', 'Metal',
                '-o', outputPath + '/OpenCL_matrixfree_lib.so',
            ], check=True)
        ...
        try:
            result = subprocess.run([
                ...
                '-framework', 'QuartzCore',
                '-framework', 'Metal',
                '-o', outputPath + '/OpenCL_matrixfree_uint16_lib.so',
            ], check=True)
        ...
        try:
            result = subprocess.run([
                ...
                '-framework', 'QuartzCore',
                '-framework', 'Metal',
                '-o', outputPath + '/OpenCL_matrixfree_uint8_lib.so',
            ], check=True)
