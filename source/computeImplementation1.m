        ...
        if options.projector_type == 1 || options.projector_type == 0
            if options.CT
                if options.subsets > 1
                    xyindex_input = xy_index(pituus(osa_iter)+1:pituus(osa_iter + 1));
                    zindex_input = z_index(pituus(osa_iter)+1:pituus(osa_iter + 1));
                else
                    xyindex_input = uint32(0);
                    zindex_input = uint16(0);
                end
%                 if exist('OCTAVE_VERSION','builtin') == 0
                    [ lor, indices, alkiot] = projector_mexCT( options, Ny, Nx, Nz, dx, dz, by, bx, bz, z_det, x, y, dy, yy, xx , NSinos, NSlices, size_x, ...
                        zmax, options.vaimennus, norm_input, single(0), pituus(osa_iter + 1) - pituus(osa_iter), attenuation_correction, normalization_correction, ...
                        randoms_correction, options.scatter, scatter_input, options.global_correction_factor, uint16(0), xyindex_input, zindex_input, NSinos, uint16(0), pseudot, det_per_ring, ...
                        TOF, TOFSize, sigma_x, TOFCenter, int64(options.TOF_bins), dec, options.verbose, nCores, ...
                        use_raw_data, uint32(2), options.listmode, ind_size, block1, blocks, uint32(0), ...
                        uint32(options.projector_type), iij, jji, kkj, uint32(options.subsets), options.angles, uint32(options.xSize), options.dPitch, ...
                        int64(numel(options.angles)));
%                 elseif exist('OCTAVE_VERSION','builtin') == 5
%                     [ lor, indices, alkiot] = projector_octCT( options, Ny, Nx, Nz, dx, dz, by, bx, bz, z_det, x, y, dy, yy, xx , NSinos, NSlices, size_x, ...
%                         zmax, options.vaimennus, norm_input, single(0), pituus(osa_iter + 1) - pituus(osa_iter), attenuation_correction, normalization_correction, ...
%                         randoms_correction, options.scatter, scatter_input, options.global_correction_factor, uint16(0), xyindex_input, zindex_input, NSinos, uint16(0), pseudot, det_per_ring, ...
%                         TOF, TOFSize, sigma_x, TOFCenter, int64(options.TOF_bins), dec, options.verbose, nCores, ...
%                         use_raw_data, uint32(2), options.listmode, ind_size, block1, blocks, uint32(0), ...
...
%                 end
                ...
                if exist('projector_mex','file') == 3
                    [ lor, indices, alkiot] = projector_mex( options, Ny, Nx, Nz, dx, dz, by, bx, bz, z_det, x, y, dy, yy, xx , NSinos, NSlices, size_x, ...
                        zmax, options.vaimennus, norm_input, single(0), pituus(osa_iter + 1) - pituus(osa_iter), attenuation_correction, normalization_correction, ...
                        randoms_correction, options.scatter, scatter_input, options.global_correction_factor, uint16(0), xy_index(pituus(osa_iter)+1:pituus(osa_iter + 1)), ...
                        z_index(pituus(osa_iter)+1:pituus(osa_iter + 1)), NSinos, uint16(0), pseudot, det_per_ring, ...
                        TOF, TOFSize, sigma_x, TOFCenter, int64(options.TOF_bins), dec, options.verbose, nCores, ...
                        use_raw_data, uint32(2), options.listmode, ind_size, block1, blocks, uint32(0), ...
                        uint32(options.projector_type), iij, jji, kkj);
%                 elseif exist('OCTAVE_VERSION','builtin') == 5 && exist('projector_oct','file') == 3
%                     [ lor, indices, alkiot] = projector_oct( options, Ny, Nx, Nz, dx, dz, by, bx, bz, z_det, x, y, dy, yy, xx , NSinos, NSlices, size_x, ...
%                         zmax, options.vaimennus, norm_input, single(0), pituus(osa_iter + 1) - pituus(osa_iter), attenuation_correction, normalization_correction, ...
...
%             if exist('OCTAVE_VERSION','builtin') == 0
                [ lor, indices, alkiot] = projector_mex( options, Ny, Nx, Nz, dx, dz, by, bx, bz, z_det, x, y, dy, yy, xx , NSinos, NSlices, size_x, ...
                    zmax, options.vaimennus, norm_input, single(0), uint32(0), attenuation_correction, normalization_correction, ...
...
%             end
    ...
    if use_raw_data
        ...
        L_input = uint16(0);
        if options.CT && (options.subsets == 1 || options.listmode)
    ...
    if options.CT
        if options.precompute_lor && options.subsets == 1 && ~options.listmode
            subsets = 2;
        else
            subsets = options.subsets;
        end
%         if exist('OCTAVE_VERSION','builtin') == 0
            [A, ll] = projector_mexCT( options, Ny, Nx, Nz, dx, dz, by, bx, bz, z_det, x, y, dy, yy, xx , NSinos, NSlices, size_x, zmax, options.vaimennus, ...
                norm_input, single(0), pituus(osa_iter + 1) - pituus(osa_iter), attenuation_correction, normalization_correction,...
                ...
                options.tube_width_z, bmin, bmax, Vmax, V, uint32(subsets), options.angles, uint32(options.xSize), options.dPitch, ...
                int64(numel(options.angles)));
%         elseif exist('OCTAVE_VERSION','builtin') == 5
%             [A, ll] = projector_octCT( options, Ny, Nx, Nz, dx, dz, by, bx, bz, z_det, x, y, dy, yy, xx , NSinos, NSlices, size_x, zmax, options.vaimennus, ...
%                 norm_input, single(0), pituus(osa_iter + 1) - pituus(osa_iter), attenuation_correction, normalization_correction,...
%                 randoms_correction, options.scatter, scatter_input, options.global_correction_factor, lor_a(pituus(osa_iter)+1:pituus(osa_iter + 1)), xy_index_input, ...
%                 z_index_input, NSinos, L_input, pseudot, det_per_ring, ...
%                 TOF, TOFSize, sigma_x, TOFCenter, int64(options.TOF_bins), dec, options.verbose, nCores, use_raw_data, uint32(0), options.listmode, lor2, summa(osa_iter), ...
...
%         if exist('OCTAVE_VERSION','builtin') == 0
            [A, ll] = projector_mex( options, Ny, Nx, Nz, dx, dz, by, bx, bz, z_det, x, y, dy, yy, xx , NSinos, NSlices, size_x, zmax, options.vaimennus, ...
                norm_input, single(0), pituus(osa_iter + 1) - pituus(osa_iter), attenuation_correction, normalization_correction,...
                randoms_correction, options.scatter, scatter_input, options.global_correction_factor, lor_a(pituus(osa_iter)+1:pituus(osa_iter + 1)), xy_index_input, ...
                z_index_input, NSinos, L_input, pseudot, det_per_ring, ...
                TOF, TOFSize, sigma_x, TOFCenter, int64(options.TOF_bins), dec, options.verbose, nCores, use_raw_data, uint32(0), options.listmode, lor2, summa(osa_iter), ...
...
%         elseif exist('OCTAVE_VERSION','builtin') == 5
%             [A, ll] = projector_oct( options, Ny, Nx, Nz, dx, dz, by, bx, bz, z_det, x, y, dy, yy, xx , NSinos, NSlices, size_x, zmax, options.vaimennus, ...
%                 norm_input, single(0), pituus(osa_iter + 1) - pituus(osa_iter), attenuation_correction, normalization_correction,...
%                 randoms_correction, options.scatter, scatter_input, options.global_correction_factor, lor_a(pituus(osa_iter)+1:pituus(osa_iter + 1)), xy_index_input, ...
%                 z_index_input, NSinos, L_input, pseudot, det_per_ring, ...
%                 TOF, TOFSize, sigma_x, TOFCenter, int64(options.TOF_bins), dec, options.verbose, nCores, use_raw_data, uint32(0), options.listmode, lor2, summa(osa_iter), ...
