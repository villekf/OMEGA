...
void OSEM_MLEM(const cl_uint &num_devices_context, const float kerroin, const int cpu_device, const cl_context &context, const cl_command_queue *commandQueues,
	...
	const cl_kernel &kernel_mlem, float *ele_ml, const size_t numel_x, const float tube_width, const float crystal_size_z, const float* x_center, 
	...
	const cl_uchar compute_norm_matrix, const bool precompute, const int32_t dec, const uint32_t projector_type) {
		...
		for (cl_uint i = 0u; i < num_devices_context; i++) {
			if (i == num_devices_context - 1u) {
				if (i == cpu_device) {
					...
					length[kk * num_devices_context + i - 1u] = meas_per_gpu[kk] + (osa_length - meas_per_gpu[kk] * static_cast<size_t>(num_devices_context - 1u) - meas_per_cpu[kk]);
					cumsum[kk * num_devices_context + i] = cumsum[kk * num_devices_context + i - 1] + length[kk * num_devices_context + i - 1u];
				}
				else {
					if (cpu_device < 0)
						length[kk * num_devices_context + i] = meas_per_gpu[kk] + (osa_length - meas_per_gpu[kk] * static_cast<size_t>(num_devices_context));
					else
						length[kk * num_devices_context + i] = meas_per_gpu[kk] + (osa_length - meas_per_gpu[kk] * static_cast<size_t>(num_devices_context - 1u) - meas_per_cpu[kk]);
				}
			}
			else {
				if (i == cpu_device) {
	...
	for (cl_uint kk = 0u; kk < subsets; kk++) {
		for (cl_uint i = 0u; i < num_devices_context; i++) {
			if (kk == 0u) {
				...
				const cl_mem siirrettavat[10] = { d_z[i] , d_x[i], d_y[i], d_xcenter[i], d_ycenter[i], d_zcenter[i], d_atten[i], d_pseudos[i], d_mlem[i], d_rhs[i] };
				status = clEnqueueMigrateMemObjects(commandQueues[i], 10, siirrettavat, NULL, 0, NULL, NULL);
				if (status != CL_SUCCESS) {
					std::cerr << getErrorString(status) << std::endl;
					return;
				}
			...
			if (normalization == 1u) {
				d_norm[kk * num_devices_context + i] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(float) * length[kk * num_devices_context + i], NULL, &status);
				if (status != CL_SUCCESS) {
					std::cerr << getErrorString(status) << std::endl;
					return;
				}
			...
			if (randoms_correction == 1u) {
				d_sc_ra[kk * num_devices_context + i] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(float) * length[kk * num_devices_context + i], NULL, &status);
				if (status != CL_SUCCESS) {
					std::cerr << getErrorString(status) << std::endl;
					return;
				}
			...
			if (precompute)
				d_lor[kk * num_devices_context + i] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(uint16_t) * length[kk * num_devices_context + i], NULL, &status);
			else
			...
			if (raw) {
				...
				d_L[kk * num_devices_context + i] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(uint16_t) * length[kk * num_devices_context + i] * 2, NULL, &status);
				if (status != CL_SUCCESS) {
					std::cerr << getErrorString(status) << std::endl;
					return;
				}
				...
				d_xyindex[kk * num_devices_context + i] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(uint32_t) * length[kk * num_devices_context + i], NULL, &status);
				if (status != CL_SUCCESS) {
					std::cerr << getErrorString(status) << std::endl;
					return;
				}
				d_zindex[kk * num_devices_context + i] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(uint16_t) * length[kk * num_devices_context + i], NULL, &status);
				if (status != CL_SUCCESS) {
					std::cerr << getErrorString(status) << std::endl;
					return;
				}
			...
			const cl_mem siirrettavat[7] = { d_Sino[kk * num_devices_context + i], d_norm[kk * num_devices_context + i], d_sc_ra[kk * num_devices_context + i], d_lor[kk * num_devices_context + i],
			d_xyindex[kk * num_devices_context + i], d_zindex[kk * num_devices_context + i], d_L[kk * num_devices_context + i]};
			status = clEnqueueMigrateMemObjects(commandQueues[i], 7, siirrettavat, NULL, 0, NULL, NULL);
			if (status != CL_SUCCESS) {
				std::cerr << getErrorString(status) << std::endl;
				return;
			}
	...
	for (cl_uint kk = 0u; kk < subsets; kk++) {
		for (cl_uint i = 0u; i < num_devices_context; i++) {
			if (kk == 0u) {
				...
				status = clEnqueueWriteBuffer(commandQueues[i], d_mlem[i], CL_FALSE, 0, sizeof(float) * im_dim, (float*)mxGetData(mxGetField(options, 0, "x0")), 0, NULL, NULL);
				//mexPrintf("status = %d\n", status);
				if (status != CL_SUCCESS) {
					std::cerr << getErrorString(status) << std::endl;
					return;
			...
			if (precompute)
				status = clEnqueueWriteBuffer(commandQueues[i], d_lor[kk * num_devices_context + i], CL_FALSE, 0, sizeof(uint16_t) * length[kk * num_devices_context + i], &lor1[cumsum[kk * num_devices_context + i]], 0, NULL, NULL);
			else
			...
			if (normalization == 1u) {
				status = clEnqueueWriteBuffer(commandQueues[i], d_norm[kk * num_devices_context + i], CL_FALSE, 0, sizeof(cl_float) * length[kk * num_devices_context + i], &norm[cumsum[kk * num_devices_context + i]], 0, NULL, NULL);
				//mexPrintf("status = %d\n", status);
				if (status != CL_SUCCESS) {
					std::cerr << getErrorString(status) << std::endl;
					return;
				...
				status = clEnqueueWriteBuffer(commandQueues[i], d_norm[kk * num_devices_context + i], CL_FALSE, 0, sizeof(cl_float) * size_norm, norm, 0, NULL, NULL);
				//mexPrintf("status = %d\n", status);
				if (status != CL_SUCCESS) {
					std::cerr << getErrorString(status) << std::endl;
					return;
			...
			if (raw) {
				...
				status = clEnqueueWriteBuffer(commandQueues[i], d_L[kk * num_devices_context + i], CL_FALSE, 0, sizeof(uint16_t) * length[kk * num_devices_context + i] * 2, &L[cumsum[kk * num_devices_context + i] * 2], 0, NULL, NULL);
				//mexPrintf("status = %d\n", status);
				if (status != CL_SUCCESS) {
					std::cerr << getErrorString(status) << std::endl;
					return;
				...
				status = clEnqueueWriteBuffer(commandQueues[i], d_xyindex[kk * num_devices_context + i], CL_FALSE, 0, sizeof(uint32_t) * length[kk * num_devices_context + i], &xy_index[cumsum[kk * num_devices_context + i]], 0, NULL, NULL);
				//mexPrintf("status = %d\n", status);
				if (status != CL_SUCCESS) {
					std::cerr << getErrorString(status) << std::endl;
					return;
				}
				status = clEnqueueWriteBuffer(commandQueues[i], d_zindex[kk * num_devices_context + i], CL_FALSE, 0, sizeof(uint16_t) * length[kk * num_devices_context + i], &z_index[cumsum[kk * num_devices_context + i]], 0, NULL, NULL);
				//mexPrintf("status = %d\n", status);
				if (status != CL_SUCCESS) {
					std::cerr << getErrorString(status) << std::endl;
					return;
	...
	//status = clEnqueueReadBuffer(commandQueues[0], d_mlem[0], CL_FALSE, 0, sizeof(float) * im_dim, &ele_ml[0], 0, NULL, NULL);

	const size_t local_size = 64ULL;
	size_t kernel_size = 0ULL;
	size_t kernel_workgroup = 0ULL;
	cl_ulong private_size = 0ULL;
	cl_ulong local_mem = 0ULL;

	status = clGetKernelWorkGroupInfo(kernel, NULL, CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE, sizeof(size_t), &kernel_size, NULL);

	mexPrintf("kernel multiple = %u\n", kernel_size);

	status = clGetKernelWorkGroupInfo(kernel, NULL, CL_KERNEL_WORK_GROUP_SIZE, sizeof(size_t), &kernel_workgroup, NULL);

	mexPrintf("kernel workgroup = %u\n", kernel_workgroup);

	status = clGetKernelWorkGroupInfo(kernel, NULL, CL_KERNEL_PRIVATE_MEM_SIZE, sizeof(size_t), &private_size, NULL);

	mexPrintf("private mem = %u\n", private_size);

	status = clGetKernelWorkGroupInfo(kernel, NULL, CL_KERNEL_LOCAL_MEM_SIZE, sizeof(size_t), &local_mem, NULL);

	mexPrintf("local mem = %u\n", local_mem);

	mexPrintf("local size = %u\n", local_size);
	...
	for (uint32_t tt = 0u; tt < Nt; tt++) {
		...
		float * Sino = (float*)mxGetData(mxGetCell(Sin, tt));
		...
		for (cl_uint kk = 0u; kk < subsets; kk++) {
			for (cl_uint i = 0u; i < num_devices_context; i++) {
				status = clEnqueueWriteBuffer(commandQueues[i], d_Sino[kk * num_devices_context + i], CL_FALSE, 0, sizeof(float) * length[kk * num_devices_context + i], &Sino[cumsum[kk * num_devices_context + i]], 0, NULL, NULL);
				if (status != CL_SUCCESS) {
					std::cerr << getErrorString(status) << std::endl;
					return;
				}
				if (randoms_correction == 1u) {
					...
					status = clEnqueueWriteBuffer(commandQueues[i], d_sc_ra[kk * num_devices_context + i], CL_FALSE, 0, sizeof(float) * length[kk * num_devices_context + i], &S_R[cumsum[kk * num_devices_context + i]], 0, NULL, NULL);
					if (status != CL_SUCCESS) {
						std::cerr << getErrorString(status) << std::endl;
						return;
					}
		...
		for (uint32_t iter = 0u; iter < Niter; iter++) {
			...
			for (uint32_t osa_iter = 0u; osa_iter < subsets; osa_iter++) {
				...
				for (cl_uint i = 0u; i < num_devices_context; i++) {
					if (atomic_64bit) {
						if (compute_norm_matrix == 0u && iter == 0u) {
							status = clEnqueueFillBuffer(commandQueues[i], d_Summ[osa_iter * num_devices_context + i], &zerou, sizeof(cl_ulong), 0, sizeof(cl_ulong) * im_dim, 0, NULL, NULL);
						}
						...
						if (compute_norm_matrix == 0u && iter == 0u) {
							status = clEnqueueFillBuffer(commandQueues[i], d_Summ[osa_iter * num_devices_context + i], &zero, sizeof(cl_float), 0, sizeof(cl_float) * im_dim, 0, NULL, NULL);
						}
						else if (compute_norm_matrix == 1u) {
				...
				for (cl_uint i = 0u; i < num_devices_context; i++) {
					...
					//mexPrintf("extra size = %u\n", (local_size - length[osa_iter * num_devices_context + i] % local_size));
					//const size_t global_size = length[osa_iter * num_devices_context + i];
					const uint64_t m_size = static_cast<uint64_t>(length[osa_iter * num_devices_context + i]);

					//mexPrintf("global size = %u\n", global_size);
					//mexPrintf("m size = %u\n", m_size);
				...
				if (num_devices_context > 1u) {
					for (cl_uint i = 1u; i < num_devices_context; i++) {
						if (atomic_64bit) {
							if (compute_norm_matrix == 1u)
								status = clEnqueueReadBuffer(commandQueues[i], d_Summ[i], CL_TRUE, 0, sizeof(cl_ulong) * im_dim, testi_summ[i - 1u].data(), 1, &events[i][0], NULL);
							else if (compute_norm_matrix == 0u && iter == 0u)
								status = clEnqueueReadBuffer(commandQueues[i], d_Summ[osa_iter * num_devices_context + i], CL_TRUE, 0, sizeof(cl_ulong) * im_dim, testi_summ[i - 1u].data(), 1, &events[i][0], NULL);
							if (status != CL_SUCCESS) {
								std::cerr << getErrorString(status) << std::endl;
								return;
							}
							if ((compute_norm_matrix == 0u && iter == 0u) || compute_norm_matrix == 1u) {
								status = clEnqueueWriteBuffer(commandQueues[0], d0_Summ[i - 1u], CL_FALSE, 0, sizeof(cl_ulong) * im_dim, testi_summ[i - 1u].data(), 0, NULL, NULL);
								if (status != CL_SUCCESS) {
									std::cerr << getErrorString(status) << std::endl;
									return;
								}
							}
							status = clEnqueueReadBuffer(commandQueues[i], d_rhs[i], CL_TRUE, 0, sizeof(cl_ulong) * im_dim, testi_rhs[i - 1u].data(), 1, &events[i][0], NULL);
							if (status != CL_SUCCESS) {
								std::cerr << getErrorString(status) << std::endl;
								return;
							}
							status = clEnqueueWriteBuffer(commandQueues[0], d0_rhs[i - 1u], CL_FALSE, 0, sizeof(cl_ulong) * im_dim, testi_rhs[i - 1u].data(), 0, NULL, NULL);
							if (status != CL_SUCCESS) {
								std::cerr << getErrorString(status) << std::endl;
								return;
							}
							...
							if (compute_norm_matrix == 1u)
								status = clEnqueueReadBuffer(commandQueues[i], d_Summ[i], CL_TRUE, 0, sizeof(float) * im_dim, testi_summ[i - 1u].data(), 1, &events[i][0], NULL);
							else if (compute_norm_matrix == 0u && iter == 0u)
								status = clEnqueueReadBuffer(commandQueues[i], d_Summ[osa_iter * num_devices_context + i], CL_TRUE, 0, sizeof(float) * im_dim, testi_summ[i - 1u].data(), 1, &events[i][0], NULL);
							if (status != CL_SUCCESS) {
								std::cerr << getErrorString(status) << std::endl;
								return;
							}
							if ((compute_norm_matrix == 0u && iter == 0u) || compute_norm_matrix == 1u) {
								status = clEnqueueWriteBuffer(commandQueues[0], d0_Summ[i - 1u], CL_FALSE, 0, sizeof(float) * im_dim, testi_summ[i - 1u].data(), 0, NULL, NULL);
								if (status != CL_SUCCESS) {
									std::cerr << getErrorString(status) << std::endl;
									return;
								}
							}
							status = clEnqueueReadBuffer(commandQueues[i], d_rhs[i], CL_TRUE, 0, sizeof(float) * im_dim, testi_rhs[i - 1u].data(), 1, &events[i][0], NULL);
							if (status != CL_SUCCESS) {
								std::cerr << getErrorString(status) << std::endl;
								return;
							}
							status = clEnqueueWriteBuffer(commandQueues[0], d0_rhs[i - 1u], CL_FALSE, 0, sizeof(float) * im_dim, testi_rhs[i - 1u].data(), 0, NULL, NULL);
							if (status != CL_SUCCESS) {
								std::cerr << getErrorString(status) << std::endl;
								return;
							}
				...
				if (compute_norm_matrix == 0u && iter == 0u && num_devices_context > 1u) {
					if (atomic_64bit) {
						status = clEnqueueReadBuffer(commandQueues[0], d_Summ[osa_iter * num_devices_context], CL_FALSE, 0, sizeof(cl_ulong) * im_dim, testi_summ[0].data(), 1, &events[0][1], &summ_event);
						if (status != CL_SUCCESS) {
							std::cerr << getErrorString(status) << std::endl;
							return;
						}
						...
						status = clEnqueueReadBuffer(commandQueues[0], d_Summ[osa_iter * num_devices_context], CL_FALSE, 0, sizeof(float) * im_dim, testi_summ[0].data(), 1, &events[0][1], &summ_event);
						if (status != CL_SUCCESS) {
							std::cerr << getErrorString(status) << std::endl;
							return;
						}
					...
					for (cl_uint i = 1u; i < num_devices_context; i++) {
						if (atomic_64bit) {
							status = clEnqueueWriteBuffer(commandQueues[0], d_Summ[osa_iter * num_devices_context + i], CL_FALSE, 0, sizeof(cl_ulong) * im_dim, testi_summ[0].data(), 1, &summ_event, NULL);
							if (status != CL_SUCCESS) {
								std::cerr << getErrorString(status) << std::endl;
								return;
							}
							...
							status = clEnqueueWriteBuffer(commandQueues[0], d_Summ[osa_iter * num_devices_context + i], CL_FALSE, 0, sizeof(float) * im_dim, testi_summ[0].data(), 1, &summ_event, NULL);
							if (status != CL_SUCCESS) {
								std::cerr << getErrorString(status) << std::endl;
								return;
							}
				...
				status = clEnqueueReadBuffer(commandQueues[0], d_mlem[0], CL_FALSE, 0, sizeof(float) * im_dim, &ele_ml[im_dim * (iter + 1u)], 1, &events[0][1], &events[0][2]);
				if (status != CL_SUCCESS) {
					std::cerr << getErrorString(status) << std::endl;
					return;
				}
				...
				for (cl_uint i = 1u; i < num_devices_context; i++) {
					status = clEnqueueWriteBuffer(commandQueues[i], d_mlem[i], CL_FALSE, 0, sizeof(cl_float) * im_dim, &ele_ml[im_dim * (iter + 1u)], 1, &events[0][2], NULL);
					if (status != CL_SUCCESS) {
						std::cerr << getErrorString(status) << std::endl;
						return;
					}
...
void f_b_project(const cl_uint &num_devices_context, const float kerroin, const int cpu_device, const cl_context &context, const cl_command_queue *commandQueues,
	...
	const size_t size_center_y, const size_t size_center_z, const bool precompute, const int32_t dec, const uint32_t projector_type) {
	...
	size_t meas_per_gpu;
	size_t meas_per_cpu = 0;
	if (cpu_device > 0 || (num_devices_context == 2 && kerroin > 0.f && kerroin != 1.f)) {
		meas_per_cpu = static_cast<size_t>(static_cast<float>(pituus[0]) / (kerroin + 1.f));
		meas_per_gpu = (pituus[0] - meas_per_cpu) / static_cast<size_t>(num_devices_context - 1);
	}
	else
	...
	for (cl_uint i = 0; i < num_devices_context; i++) {
		if (i == num_devices_context - 1) {
			if (i == cpu_device) {
				...
				length[i - 1] = meas_per_gpu + (pituus[0] - meas_per_gpu * static_cast<size_t>(num_devices_context - 1) - meas_per_cpu);
				cumsum[i] = cumsum[i - 1] + length[i - 1];
			}
			else {
				if (cpu_device < 0)
					length[i] = meas_per_gpu + (pituus[0] - meas_per_gpu * static_cast<size_t>(num_devices_context));
				else
					length[i] = meas_per_gpu + (pituus[0] - meas_per_gpu * static_cast<size_t>(num_devices_context - 1) - meas_per_cpu);
			}
		}
		else {
			if (i == cpu_device || (num_devices_context == 2 && kerroin > 0.f && kerroin != 1.f && i == 0)) {
				length[i] = meas_per_cpu;
			}
			else
	...
	cl_mem* d0_output = (cl_mem*)malloc((num_devices_context - 1) * sizeof(cl_mem));
	cl_mem* d0_Summ = (cl_mem*)malloc((num_devices_context - 1) * sizeof(cl_mem));
	...
	if (num_devices_context > 1u) {
		for (cl_uint i = 1; i < num_devices_context; i++) {
			...
			status = clEnqueueReadBuffer(commandQueues[i], d_output[i], CL_TRUE, 0, sizeof(float) * size_rhs, testi_rhs[i - 1].data(), 1, &events[i][0], NULL);
			if (status != CL_SUCCESS) {
				std::cerr << getErrorString(status) << std::endl;
				return;
			}
			...
			status = clEnqueueWriteBuffer(commandQueues[0], d0_output[i - 1], CL_FALSE, 0, sizeof(float) * size_rhs, testi_rhs[i - 1].data(), 0, NULL, NULL);
			if (status != CL_SUCCESS) {
				std::cerr << getErrorString(status) << std::endl;
				return;
			}
	...
	for (cl_uint i = 1; i < num_devices_context; i++) {
		...
		status = clEnqueueNDRangeKernel(commandQueues[0], kernel_sum, 1, NULL, &size_rhs, NULL, 0, NULL, NULL);
		if (status != CL_SUCCESS) {
			std::cerr << getErrorString(status) << std::endl;
			return;
		}
...
cl_int clGetPlatformsContext(const uint32_t device, const float kerroin, cl_context& context, size_t& size, int& cpu_device, 
	...
	//cl_device_id * devices = (cl_device_id*)alloca((num_devices_context - n_ignores) * sizeof(cl_device_id));
	uint32_t ll = 0u;
	...
	//for (size_t i = 0; i < num_devices_context; i++) {
	//	size_t koko = 0u;
	//	clGetDeviceInfo(devices2[i], CL_DEVICE_EXTENSIONS, NULL, NULL, &koko);
	//	char* ext = new char [koko];
	//	clGetDeviceInfo(devices2[i], CL_DEVICE_EXTENSIONS, koko, ext, NULL);
	//	std::string str(ext);
	//	std::string str2("cl_khr_int64_base_atomics");
	//	std::size_t found = str.find(str2);
	//	//if (found == std::string::npos) {
	//	//	atomic_64bit = false;
	//		mexPrintf("extensions: %s\n", ext);
	//	//}
	//	delete[] ext;
	//}
...
cl_int ClBuildProgramGetQueues(cl_program& program, const char* k_path, const cl_context context, const cl_uint num_devices_context, const cl_device_id* devices,
	...
	//mexPrintf("device_copy\n");

	//std::string I_header(header_directory);
	//I_header = "-I" + I_header;

	mexPrintf("Header folder = %s\n", header_directory);

	mexPrintf("kernel folder = %s\n", k_path);

	//std::string header_source = header_to_string(header_directory);
	//content_atom += header_source;
	...
	if (atomic_64bit) {
		...
		//if (projector_type == 2u)
		//	kernel_path_atom += "orth_64atom.cl";
		//else
	...
	if (status != CL_SUCCESS) {
		...
		//if (projector_type == 2u)
		//	kernel_path += "orth.cl";
		//else if (projector_type == 50u)
			kernel_path += ".cl";
		//else
			//kernel_path += "siddon.cl";

		mexPrintf("kernel_path = %s\n", kernel_path.c_str());
	...
	//cl_command_queue *commandQueues = (cl_command_queue*)alloca(sizeof(cl_command_queue) * size);
	for (size_t i = 0; i < num_devices_context; i++) {
...
void precomp_siddon(const cl_uint &num_devices_context, const cl_context &context, const cl_command_queue *commandQueues,
