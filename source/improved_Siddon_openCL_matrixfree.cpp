...
void reconstruction(const size_t koko, const uint16_t* lor1, const float* z_det, const float* x, const float* y, const float* Sin, const int Nx, const int Ny, 
	...
	const float zmax, const float xxv, const float yyv, const float xxa, const float yya, const float NSlices, const int* pituus, const uint64_t* summa, 
	const size_t koko_l, const int* xy_index, const int* z_index, const int size_x, const int TotSinos, float* mlem, float* osem, const bool verbose, 
	const int attenuation_correction, const float* atten, const int size_atten, const int subsets, const float epps, const int* maksimi, const bool* rekot, 
	const char* k_path) {

	array lor(koko_l, lor1, afHost);

	array zindex(koko, z_index, afHost);
	array xindex(koko, xy_index, afHost);
	...
	kernel_path = k_path + std::string("\\siddon_kernel_matrixfree.cl");
#else
	kernel_path = k_path + std::string("/siddon_kernel_matrixfree.cl");
#endif
	...
	for (int iter = 0; iter < Niter; iter++) {
		...
		for (int osa_iter = 0; osa_iter < subsets; osa_iter++) {
			...
			array sub_index_array, apu_lor, xo, zo, uu;

			if (subsets > 1) {

				sub_index_array = range(dim4(length), 0, s32) + pituus[osa_iter];
			...
			cl_mem * d_xyindex = xo.device<cl_mem>();
			...
			clSetKernelArg(kernel, 0, maksimi[osa_iter] * sizeof(float), NULL);
			clSetKernelArg(kernel, 1, maksimi[osa_iter] * sizeof(float), NULL);
			...
			clSetKernelArg(kernel, 31, sizeof(cl_mem), d_xyindex);
			clSetKernelArg(kernel, 32, sizeof(cl_mem), d_zindex);
			...
			clSetKernelArg(kernel, 39, sizeof(cl_mem), d_rhs);
			clSetKernelArg(kernel, 40, sizeof(cl_mem), d_Summ);
			...
			if (rekot[0])
				pz_ml(span, iter + 1) = pz_ml(span, iter) / (Summ + epps) * rhs;
...

{
	...
	uint64_t* summa = (uint64_t*)mxGetData(prhs[32]);

	int* xy_index = (int*)mxGetData(prhs[33]);
	size_t koko = mxGetNumberOfElements(prhs[33]);

	int* z_index = (int*)mxGetData(prhs[34]);
	...
	reconstruction(koko, lor1, z_det, x, y, Sin, Nx, Ny, Nz, Niter, x0, d, dz, bxf, byf, bzf, bxb, byb, bzb, bx, by, bz, maxxx, maxyy, minxx, minyy, zmax, xxv, 
		yyv, xxa, yya, NSlices, pituus, summa, koko_l, xy_index, z_index, size_x, TotSinos, ele_ml, ele_os, verbose, attenuation_correction, atten, size_atten, 
		subsets, epps, maksimi, rekot, k_path);
