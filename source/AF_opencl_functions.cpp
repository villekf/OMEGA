...
void update_opencl_inputs(AF_im_vectors& vec, OpenCL_im_vectors& vec_opencl, const bool mlem, const uint32_t im_dim, const uint32_t n_rekos,
	const uint32_t n_rekos_mlem, const RecMethods MethodList, const bool atomic_64bit, const bool atomic_32bit, const bool use_psf, 
	const uint32_t nMAPOS, cl::CommandQueue& af_queue, const scalarStruct& inputScalars)
{

	const cl::detail::size_t_array origin = { 0, 0, 0 };
	if (mlem) {
		cl::detail::size_t_array region = { inputScalars.Nx, inputScalars.Ny, inputScalars.Nz * n_rekos_mlem };
		if (use_psf)
		...
		if (atomic_64bit)
			vec.rhs_mlem = af::constant(std::int64_t{0}, static_cast<size_t>(im_dim) * n_rekos_mlem, 1, s64);
		...
		cl_int status = af_queue.enqueueCopyBufferToImage(vec_opencl.d_im_mlem, vec_opencl.d_image_mlem, 0, origin, region);
		...
		cl::detail::size_t_array region = { inputScalars.Nx, inputScalars.Ny, inputScalars.Nz * n_rekos };
		if (use_psf)
		...
		if (atomic_64bit)
			vec.rhs_os = af::constant(std::int64_t{0}, static_cast<size_t>(im_dim) * static_cast<size_t>(n_rekos), 1, s64);
		else if (atomic_32bit)
		...
		af_queue.finish();
		cl_int status = af_queue.enqueueCopyBufferToImage(vec_opencl.d_im_os, vec_opencl.d_image_os, 0, origin, region);
		if (status != CL_SUCCESS) {
			getErrorString(status);
			mexPrintf("Image copy failed\n");
			mexEvalString("pause(.0001);");
...
cl_int createKernels(cl::Kernel& kernel_ml, cl::Kernel & kernel, cl::Kernel& kernel_mramla, cl::Kernel& kernelNLM, cl::Kernel& kernelMed, 
	cl::Kernel& kernelFP, cl::Kernel& kernelBP, const bool osem_bool, const cl::Program &program_os, const cl::Program& program_ml, const cl::Program& program_mbsrem,
	const cl::Program& programProj4, const cl::Program& programProj5, const cl::Program& programAux, const cl::Program& programOSFP, 
	const cl::Program& programOSBP, const RecMethods MethodList, const Weighting w_vec, const uint32_t projector_type, const bool mlem_bool, 
	const scalarStruct inputScalars)
{
	cl_int status = CL_SUCCESS;
	// Kernel for the OS-methods (OSEM, RAMLA, RBI, BSREM, etc.)
	if (projector_type == 4) {
		kernelFP = cl::Kernel(programProj4, "projectorType4Forward", &status);
		if (status != CL_SUCCESS) {
			getErrorString(status);
		...
		kernelBP = cl::Kernel(programProj4, "projectorType4Backward", &status);
		if (status != CL_SUCCESS) {
			getErrorString(status);
	...
	if (projector_type == 5) {
		kernelFP = cl::Kernel(programProj4, "projectorType5Forward", &status);
		if (status != CL_SUCCESS) {
			getErrorString(status);
		...
		kernelBP = cl::Kernel(programProj4, "projectorType5Backward", &status);
		if (status != CL_SUCCESS) {
			getErrorString(status);
	...
	if (osem_bool && projector_type != 4 && projector_type != 5 && projector_type != 6) {


		if (projector_type == 2u || projector_type == 3u || (projector_type == 1u && ((inputScalars.precompute || (inputScalars.n_rays * inputScalars.n_rays3D) == 1)))) {
	...
	if (mlem_bool && projector_type != 4 && projector_type != 5 && projector_type != 6) {
	...
	if ((MethodList.MRAMLA || MethodList.MBSREM || MethodList.RBIOSL || MethodList.RBI || MethodList.PKMA) && (w_vec.MBSREM_prepass ||
		MethodList.COSEM || MethodList.ACOSEM || MethodList.ECOSEM || MethodList.OSLCOSEM > 0) && projector_type != 4 && projector_type != 5 && projector_type != 6) {
	...
	if (nRekos > 0 && inputScalars.projector_type < 4) {
		if (projector_type == 2u || projector_type == 3u || (projector_type == 1u && ((inputScalars.precompute || (inputScalars.n_rays * inputScalars.n_rays3D) == 1))))
			kernelFP = cl::Kernel(programOSFP, "kernel_multi", &status);
		else
			kernelFP = cl::Kernel(programOSFP, "siddon_multi", &status);
		if (status != CL_SUCCESS) {
			getErrorString(status);
			mexPrintf("Failed to create projection type 1-3 FP kernel\n");
			return status;
		}
		else if (DEBUG) {
			mexPrintf("OpenCL kernel successfully created\n");
		...
		if (projector_type == 2u || projector_type == 3u || (projector_type == 1u && ((inputScalars.precompute || (inputScalars.n_rays * inputScalars.n_rays3D) == 1))))
			kernelBP = cl::Kernel(programOSBP, "kernel_multi", &status);
		else
			kernelBP = cl::Kernel(programOSBP, "siddon_multi", &status);
		if (status != CL_SUCCESS) {
			getErrorString(status);
			mexPrintf("Failed to create projection type 1-3 BP kernel\n");
			return status;
		}
		else if (DEBUG) {
			mexPrintf("OpenCL kernel successfully created\n");
...
cl_int createAndWriteBuffers(std::vector<cl::Buffer>& d_x, std::vector<cl::Buffer>& d_z, cl::Buffer& d_x_mlem, cl::Buffer& d_z_mlem, 
	std::vector<cl::Buffer>& d_lor, std::vector<cl::Buffer>& d_L, std::vector<cl::Buffer>& d_zindex,
	...
	const uint32_t TotSinos, const size_t size_atten, const size_t size_norm, const size_t size_scat, const uint32_t prows, std::vector<size_t>& length, 
	const float* x, const float* z_det, const uint32_t* xy_index, const uint16_t* z_index, const uint16_t* lor1, const uint16_t* L, const float* Sin, 
	const uint8_t raw, cl::Context& af_context, const uint32_t subsets, const int64_t* pituus, const float* atten, const float* norm, const float* scat,
	const float* V,	cl::CommandQueue& af_queue, cl::Image3D& d_atten, std::vector<cl::Buffer>& d_norm, std::vector<cl::Buffer>& d_scat, cl::Buffer& d_V, 
	cl::Buffer& d_xcenter, 	cl::Buffer& d_ycenter, cl::Buffer& d_zcenter, const float* x_center, const float* y_center, const float* z_center, 
	...
	const bool atomic_64bit, const bool atomic_32bit, const bool randoms_correction, const mxArray* sc_ra, const bool precompute,
	cl::Buffer& d_lor_mlem, cl::Buffer& d_L_mlem, cl::Buffer& d_zindex_mlem, cl::Buffer& d_xyindex_mlem, cl::Buffer& d_Sino_mlem, 
	cl::Buffer& d_sc_ra_mlem, cl::Buffer& d_reko_type, cl::Buffer& d_reko_type_mlem, const bool osem_bool,	const bool mlem_bool, const size_t koko, 
	const uint8_t* reko_type, const uint8_t* reko_type_mlem, const uint32_t n_rekos, const uint32_t n_rekos_mlem, cl::Buffer& d_norm_mlem, 
	cl::Buffer& d_scat_mlem, const scalarStruct& inputScalars, const bool loadTOF, cl::Buffer& d_TOFCenter, const float* TOFCenter, 
	const uint32_t subsetsUsed, const uint32_t osa_iter0, const Weighting& w_vec, cl::Image2D& d_maskFP,
	cl::Image2D& d_maskBP, const uint8_t listmode, const bool CT)
{
	cl_int status = CL_SUCCESS;
	...
	if (!CT && listmode == 0) {
		d_x[0] = cl::Buffer(af_context, CL_MEM_READ_ONLY, sizeof(float) * size_of_x, NULL, &status);
		if (status != CL_SUCCESS) {
			getErrorString(status);
			return status;
		}
	...
	if (inputScalars.maskFP || inputScalars.maskBP) {
		...
		if (inputScalars.maskFP) {
			...
			d_maskFP = cl::Image2D(af_context, CL_MEM_READ_ONLY, format, imX, imY, 0, NULL, &status);
		}
		if (inputScalars.maskBP) {
			...
			d_maskBP = cl::Image2D(af_context, CL_MEM_READ_ONLY, format, imX, imY, 0, NULL, &status);
		}
		if (status != CL_SUCCESS) {
			getErrorString(status);
			return status;
	...
	if (osem_bool) {
		...
		for (uint32_t kk = osa_iter0; kk < subsetsUsed; kk++) {
			...
			if (inputScalars.TOF) {
				if (loadTOF) {
					if ((inputScalars.PET) && listmode == 0)
				...
				if ((inputScalars.PET || CT || inputScalars.SPECT) && listmode == 0)
					d_Sino[kk] = cl::Buffer(af_context, CL_MEM_READ_ONLY, sizeof(float) * length[kk] * w_vec.size_x * w_vec.size_y, NULL, &status);
				else
	...
	if (mlem_bool) {
		if (CT && listmode != 1) {
			if (inputScalars.PITCH)
				d_z_mlem = cl::Buffer(af_context, CL_MEM_READ_ONLY, sizeof(float) * w_vec.nProjections * 6, NULL, &status);
			else
				d_z_mlem = cl::Buffer(af_context, CL_MEM_READ_ONLY, sizeof(float) * w_vec.nProjections * 2, NULL, &status);
			if (status != CL_SUCCESS) {
				getErrorString(status);
				return status;
			}
		...
		if (CT || listmode > 0) {
			d_x_mlem = cl::Buffer(af_context, CL_MEM_READ_ONLY, sizeof(float) * size_of_x, NULL, &status);
			if (status != CL_SUCCESS) {
				getErrorString(status);
				return status;
			}
		...
		if (inputScalars.TOF && listmode != 2)
			d_Sino_mlem = cl::Buffer(af_context, CL_MEM_READ_ONLY, sizeof(float) * koko * inputScalars.nBins, NULL, &status);
		else if (listmode != 2)
	...
	if (!CT && listmode == 0) {
		status = af_queue.enqueueWriteBuffer(d_x[0], CL_FALSE, 0, sizeof(float) * size_of_x, x);
		if (status != CL_SUCCESS) {
			getErrorString(status);
			return status;
		}
		...
		for (uint32_t kk = osa_iter0; kk < subsetsUsed; kk++) {
			if (CT && listmode != 1) {
				...
				if (CT || inputScalars.PET)
					...
					status = af_queue.enqueueWriteBuffer(d_z[kk], CL_FALSE, 0, sizeof(float) * w_vec.nProjections * 2, z_det);
				if (status != CL_SUCCESS) {
					getErrorString(status);
					return status;
				}
					...
					for (int64_t to = std::int64_t{0}; to < inputScalars.nBins; to++)
						if ((inputScalars.PET) && listmode == 0)
							status = af_queue.enqueueWriteBuffer(d_Sino[kk], CL_FALSE, sizeof(float) * length[kk] * to * w_vec.size_x * w_vec.size_y,
								sizeof(float) * length[kk], &Sin[pituus[kk] * w_vec.size_x * w_vec.size_y + koko * to]);
						else
			...
			else if (listmode != 2 && inputScalars.projector_type < 4) {
				if ((inputScalars.PET || CT || inputScalars.SPECT) && listmode == 0)
	...
	if (mlem_bool) {
		...
		if (CT && listmode != 1) {
			if (inputScalars.PITCH)
				status = af_queue.enqueueWriteBuffer(d_z_mlem, CL_FALSE, 0, sizeof(float) * w_vec.nProjections * 6, z_det);
			else
				status = af_queue.enqueueWriteBuffer(d_z_mlem, CL_FALSE, 0, sizeof(float) * w_vec.nProjections * 2, z_det);
			if (status != CL_SUCCESS) {
				getErrorString(status);
				return status;
			}
		...
		if (CT || listmode > 0) {
			status = af_queue.enqueueWriteBuffer(d_x_mlem, CL_FALSE, 0, sizeof(float) * w_vec.nProjections * 6, x);
			if (status != CL_SUCCESS) {
				getErrorString(status);
				return status;
			}
		...
		if (inputScalars.TOF)
			status = af_queue.enqueueWriteBuffer(d_Sino_mlem, CL_FALSE, 0, sizeof(float) * koko * inputScalars.nBins, Sin);
		else if (listmode != 2)
...
cl_int createProgram(const bool verbose, const char* k_path, cl::Context& af_context, cl::Device& af_device_id, const char* fileName, 
	cl::Program& program_os, cl::Program& program_ml, cl::Program& program_mbsrem, cl::Program& programProj4, cl::Program& programProj5, 
	cl::Program& programAux, cl::Program& programOSFP, cl::Program& programOSBP, bool& atomic_64bit, const bool atomic_32bit, const uint32_t device, const char* header_directory,
	scalarStruct inputScalars, const bool find_lors, const RecMethods MethodList, const bool osem_bool, const bool mlem_bool, 
	const uint32_t n_rekos, const uint32_t n_rekos_mlem, const Weighting& w_vec, const uint32_t osa_iter0, const uint32_t scatter, 
	const uint8_t listmode, const bool CT, const size_t local_size[]) {
	...
	if (osem_bool && inputScalars.projector_type != 4 && inputScalars.projector_type != 5 && inputScalars.projector_type != 6) {
	...
	if (mlem_bool && inputScalars.projector_type != 4 && inputScalars.projector_type != 5 && inputScalars.projector_type != 6) {
		std::string ml_options = options;
		if ((inputScalars.projector_type == 2 || inputScalars.projector_type == 3u || inputScalars.TOF) && inputScalars.dec > 0)
			ml_options += (" -DDEC=" + std::to_string(inputScalars.dec));
	...
	if (inputScalars.projector_type == 4) {
		options += " -DFP";
		options += " -DBP";
		status = buildProgram(verbose, content, af_context, af_device_id, programProj4, atomic_64bit, atomic_32bit, options);
	}
	else if (inputScalars.projector_type == 5) {
		options += " -DFP";
		options += " -DBP";
		status = buildProgram(verbose, content, af_context, af_device_id, programProj5, atomic_64bit, atomic_32bit, options);
	}
	...
	if (nRekos > 0 && inputScalars.projector_type < 4) {
		...
		//status = buildProgram(verbose, k_path, af_context, af_device_id, program_os, atomic_64bit, os_options);
		status = buildProgram(verbose, content, af_context, af_device_id, programOSFP, atomic_64bit, atomic_32bit, os_optionsFP);
		std::string os_optionsBP = os_options;
		os_optionsBP += " -DBP";
		status = buildProgram(verbose, content, af_context, af_device_id, programOSBP, atomic_64bit, atomic_32bit, os_optionsBP);
	}
	if (MethodList.NLM || MethodList.MRP) {
		...
		status = buildProgram(verbose, contentAux, af_context, af_device_id, programAux, atomic_64bit, atomic_32bit, options);
	}
	return status;
}
