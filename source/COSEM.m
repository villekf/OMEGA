function x_cosem = COSEM(options)
%COSEM Complete data OSEM.
%   Implements the COSEM reconstruction on input PET data.
%   See main_nongate.m for options-variables.
%
%   x_cosem = COSEM(options) returns the COSEM reconstructions for all
...
x_cosem = zeros(options.Nx,options.Ny,options.Nz, options.Niter + 1);
x_cosem(:,:,:,1) = options.x0;
x_cosem = reshape(x_cosem, options.Nx*options.Ny*options.Nz, options.Niter + 1);
...
for ll = 1 : options.subsets
    ...
    if options.use_fsparse == false
        if options.precompute_lor == false
            C_aco(:,ll) = full(sum(spdiags(uu./(A*x_cosem(:,1)+epps),0,size(A,1),size(A,1))*(A.*(x_cosem(:,1)')))');
        else
            C_aco(:,ll) = full(sum(spdiags(uu./(A*x_cosem(:,1)+epps),0,size(A,1),size(A,1))*(A.*(x_cosem(:,1)')))');
        end
    else
        [I, ~, VV] = find((uu./(A*x_cosem(:,1)+epps)));
        if options.precompute_lor == false
            C_aco(:,ll) = full(sum(fsparse(I, I, VV, [size(A,1) size(A,1) length(VV)])*(A.*(x_cosem(:,1)')))');
        else
            C_aco(:,ll) = full(sum(fsparse(I, I, VV, [size(A,1) size(A,1) length(VV)])*(A.*(x_cosem(:,1)')))');
        end
    end
...
for ii = 1 : options.Niter
    ...
    for kk = 1 : options.subsets
        ...
        if options.use_fsparse
            [I, ~, VV] = find((uu./(A*co_apu+epps)));
            if options.precompute_lor == false
                C_aco(:,kk) = full(sum(fsparse(I, I, VV, [size(A,1) size(A,1) length(VV)])*(A.*(co_apu')))');
            else
                C_aco(:,kk) = full(sum(fsparse(I, I, VV, [size(A,1) size(A,1) length(VV)])*(A.*(co_apu')))');
            end
        else
            if options.precompute_lor == false
                C_aco(:,kk) = full(sum(spdiags(uu./(A*co_apu+epps),0,size(A,1),size(A,1))*(A.*(co_apu')))');
            else
                C_aco(:,kk) = full(sum(spdiags(uu./(A*co_apu+epps),0,size(A,1),size(A,1))*(A.*(co_apu')))');
            end
        end
        apu = (sum(C_aco,2)./D);
        co_apu = (apu)*sum(uu)/sum(A*apu+epps);
        tElapsed = toc(tStart);
        disp(['COSEM sub-iteration ' num2str(kk) ' took ' num2str(tElapsed) ' seconds'])
        disp(['COSEM sub-iteration ' num2str(kk) ' finished'])
    end
    x_cosem(:,ii+1) = co_apu;
end
x_cosem = reshape(x_cosem,options.Nx,options.Ny,options.Nz, options.Niter + 1);
