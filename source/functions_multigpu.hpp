...
void OSEM_MLEM(const cl_uint& num_devices_context, const float kerroin, const int cpu_device, const cl::Context& context, const std::vector<cl::CommandQueue>& commandQueues,
	...
	const cl_uchar compute_norm_matrix, mxArray* cell, const bool osem_bool, const float* V,
...
void f_b_project(const cl_uint& num_devices_context, const float kerroin, const int cpu_device, const cl::Context& context, const std::vector<cl::CommandQueue> & commandQueues,
	const size_t koko, const uint16_t* lor1, const float* z_det, const float* x, const float* y, const float* rhs, const mxArray* sc_ra, scalarStruct inputScalars, 
	const int64_t* pituus, const size_t koko_l, const uint32_t* xy_index,
	const uint16_t* z_index, const uint32_t TotSinos, const bool verbose, const float* atten, const size_t size_atten, const float* norm, const size_t size_norm,
	const uint32_t* pseudos, const uint16_t* L, const uint32_t im_dim,
	...
	const size_t size_center_y, const size_t size_center_z, const mxArray* Sin, const bool atomic_64bit, const bool atomic_32bit, 
	const float* V, const size_t size_V, const size_t local_size[], const mxArray* options, 
...
void reconstruction_multigpu(const size_t koko, const uint16_t* lor1, const float* z_det, const float* x, const float* y, const mxArray* Sin, const mxArray* sc_ra, 
	const mxArray* options, scalarStruct inputScalars, const int64_t* pituus, const size_t koko_l, const uint32_t* xy_index, const uint16_t* z_index, const uint32_t TotSinos,
	...
	const size_t size_rekot, const uint32_t* pseudos, const uint16_t* L, const bool osem_bool, const char* fileName, const uint32_t device, float kerroin, 
...
void reconstruction_f_b_proj(const size_t koko, const uint16_t* lor1, const float* z_det, const float* x, const float* y, const float* rhs, const mxArray* sc_ra,
	scalarStruct inputScalars, const int64_t* pituus, const size_t koko_l, const uint32_t* xy_index, const uint16_t* z_index, const uint32_t TotSinos, const bool verbose, 
	const float* atten, const size_t size_atten, const float* norm, const size_t size_norm, const char* k_path, const uint32_t* pseudos, const uint16_t* L, 
