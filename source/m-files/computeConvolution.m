...
%COMPUTECONVOLUTION Computes symmetrically padded 2D/3D convolution for the
%input vector (image). 2D or 3D is determined from the number and type of
%input variables.
%   
% Examples:
%   vec = computeConvolution(vec, options, Nx, Ny, Nz, gaussK);
%   vec = computeConvolution(vec, g_dim_x, g_dim_y, g_dim_z, Nx, Ny, Nz, gaussK);
%   vec = computeConvolution(vec, g_dim_x, g_dim_y, Nx, Ny, gaussK);
%
% INPUTS:
%   vec = The image estimes
%   g_dim_x = Size of the PSF kernel in x-direction
%   g_dim_y = Size of the PSF kernel in y-direction
%   g_dim_z = Size of the PSF kernel in z-direction (omit when 2D)
...
if nargin >= 8
    ...
    Nz = varargin{6};
    gaussK = varargin{7};
elseif ~isstruct(varargin{1}) && nargin == 6
    g_dim_x = varargin{1};
    g_dim_y = varargin{2};
    Nx = varargin{3};
    Ny = varargin{4};
...
if ~ismatrix(gaussK)
    ...
    gaussK = reshape(gaussK, g_dim_x * 2 + 1, g_dim_y * 2 + 1);
    vec = padding(vec, [g_dim_x g_dim_y]);
end
