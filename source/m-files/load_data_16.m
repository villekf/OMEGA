function [varargout] = load_data_16(options)
%% Load Siemens Biograph 16 PET data
...
%
% See also form_sinograms, load_data, load_data_Vision
...
if options.use_machine == 1 || options.use_machine == 3
    ...
    if exist(options.fpath,'file') ~= 2
        [options.file, options.fpath] = uigetfile({'*.ptd;*.l;*.lst'},'Select 16 list-mode datafile');
    ...
    if options.use_machine == 1
        ...
        % Load the data
        [raw_SinM, SinD, LL1, LL2, tpoints, DD1, DD2] = list2sinogram16(nimi, partitions, (alku), (loppu), logical(options.randoms_correction), v_size, store_coordinates, ...
        ...
        if store_coordinates
            ...
            z_length = single((options.rings + 2) * options.cr_pz);
            z = linspace(single(0), z_length, options.rings + 1 + 2)';
            z = z(2:end) - z(2) / 2;
            z([14;28]) = [];
            z = z - options.axial_fov/2 + (options.axial_fov - (options.cr_pz*(options.rings + 2)))/2;
            % if min(z(:)) == 0
    ...
    elseif options.use_machine == 3
        ...
        [raw_SinM, SinDelayed] = list2sinogram16(nimi, partitions, (alku), (loppu), logical(options.randoms_correction), v_size, options.store_raw_data, ...
        ...
        if Nt == 1
            ...
            for kk = 1 : Nt
                ...
                if options.randoms_correction
                    SinDelayed{kk} = permute(reshape(apu_d, options.Nang, options.Ndist, options.TotSinos), [2 1 3 4]);
                end
                if (~options.corrections_during_reconstruction && (options.randoms_correction || options.normalization_correction)) || ...
                        options.fill_sinogram_gaps && (sum(options.pseudot) > 0 || options.det_per_ring < options.det_w_pseudo)
                    Sino = single(raw_SinM{kk});
                    if ~options.corrections_during_reconstruction && options.randoms_correction
                        Sino = Sino - single(SinDelayed{kk}) / options.TOF_bins;
                        appliedCorrections.randoms = true;
                    end
    ...
    if exist(options.fpath,'file') ~= 2
        [options.file, options.fpath] = uigetfile({'*.ptd;*.s'},'Select 16 sinogram datafile');
        if isequal(options.file, 0)
            error('No file was selected!')
        end
        nimi = [options.fpath options.file];
