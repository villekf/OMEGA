...
%
%   Two types of interpolation are available. The built-in fillmissing,
%   fillmissing2 (2023a and onwards only) or the function inpaint_nans from
%   File exchange: 
%   https://se.mathworks.com/matlabcentral/fileexchange/4551-inpaint_nans
%   Note that on Octave, only inpaint_nans is expected to work.
%
% INPUTS:
...
%
% See also fillmissing, fillmissing2, inpaint_nans, form_sinograms


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Copyright (C) 2020-2024 Ville-Veikko Wettenhovi, Samuli Summala
%
...
if options.verbose
    disp('Starting sinogram gap filling. This can take several minutes.')
end
...
if strcmp('fillmissing',options.gap_filling_method) || strcmp('fillmissing2',options.gap_filling_method)
    Sin = single(Sin);
    for jj = 1 : size(Sin,5)
        for uu = 1 : size(Sin,4)
            for kk = 1 : size(Sin,3)
                apu = Sin(:,:,kk,uu,jj);
                apu(gaps) = NaN;
                if exist('fillmissing2','file') == 2 && strcmp('fillmissing2',options.gap_filling_method)
                    jelppi1 = fillmissing2(apu, options.interpolation_method_fillmissing);
                    jelppi2 = fillmissing2(apu', options.interpolation_method_fillmissing)';
                else
