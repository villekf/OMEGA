...
if layer1 == 1
    x1 = xp(1 : det_w_pseudo);
    y1 = yp(1 : det_w_pseudo);
    xx1 = x1(L(:,1));
    yy1 = y1(L(:,1));
elseif layer1 == 2
    x1 = xp(det_w_pseudo + 1 : end);
    y1 = yp(det_w_pseudo + 1 : end);
    xx1 = x1(L(:,1));
    yy1 = y1(L(:,1));
end
if layer2 == 1
    x2 = xp(1 : det_w_pseudo);
    y2 = yp(1 : det_w_pseudo);
    xx2 = x2(L(:,2));
    yy2 = y2(L(:,2));
elseif layer2 == 2
    x2 = xp(det_w_pseudo + 1 : end);
    y2 = yp(det_w_pseudo + 1 : end);
    xx2 = x2(L(:,2));
    yy2 = y2(L(:,2));
end
...
% Remove NaN values
if sum(isnan(x(:))) > 0
    if exist('fillmissing2','file') == 2
        x = fillmissing2(x,'linear');
        y = fillmissing2(y,'linear');
        x2 = fillmissing2(x2,'linear');
        y2 = fillmissing2(y2,'linear');
    elseif exist('fillmissing','file') == 2
        x = fillmissing(x,'linear');
        y = fillmissing(y,'linear');
        x2 = fillmissing(x2,'linear');
        y2 = fillmissing(y2,'linear');
    else
        x = fillVal(x, Ndist, Nang);
        y = fillVal(y, Ndist, Nang);
        x2 = fillVal(x2, Ndist, Nang);
        y2 = fillVal(y2, Ndist, Nang);
    end
end
...
function out = fillVal(x, Ndist, Nang)
missing = isnan(x(:));
X = 1:numel(x(:));
X0 = X(~missing);
Y0 = Y(~missing);
Xi = X(missing);
Yi = interp1(X0, Y0, Xi, 'linear');
out = Y;
out(missing) = Yi;
out = reshape(out, Ndist, Nang);
end