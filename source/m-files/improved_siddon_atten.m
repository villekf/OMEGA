...
%         continue
        ...
        if abs(y_diff)<1e-8
            ...
            if yd<= max(yy) && yd>=min(yy)
                ...
                tempi = int32(apuX1:apuX2)';
                templ_ijk = ones(numel(tempi),1,'double')*d;
                tempj = ones(numel(tempi),1,'int32')*(int32(apu));
                templ_ijk(1) = dT1;
                templ_ijk(end) = dT2;
                ...
                if attenuation_correction
                    ...
                    alkiot{lo}=double(templ_ijk(tempi));
                end
        ...
        elseif abs(x_diff)<1e-8
            if xd<= max(xx) && xd>=min(xx)
                ...
                tempj = int32(apuY1:apuY2)';
                templ_ijk = ones(numel(tempj),1,'double')*d;
                tempi = ones(numel(tempj),1,'int32')*(int32(apu));
                templ_ijk(1) = dT1;
                templ_ijk(end) = dT2;
