...
if options.TV_use_anatomical
    ...
    alkuarvo = alkuarvo - min(alkuarvo(:));
    alkuarvo = alkuarvo / max(alkuarvo(:));
    if options.TVtype == 1
        S = assembleS(alkuarvo,options.T,options.Ny,options.Nx,options.Nz);
        if options.implementation == 2
            s1 = single(S(1:3:end,1));
            s2 = single(S(1:3:end,2));
            s3 = single(S(1:3:end,3));
            s4 = single(S(2:3:end,1));
            s5 = single(S(2:3:end,2));
            s6 = single(S(2:3:end,3));
            s7 = single(S(3:3:end,1));
            s8 = single(S(3:3:end,2));
            s9 = single(S(3:3:end,3));
            clear S
            TVdata.s = [s1(:);s2(:);s3(:);s4(:);s5(:);s6(:);s7(:);s8(:);s9(:);];
