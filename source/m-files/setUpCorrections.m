...
if options.useEFOV
    ...
    if options.useMultiResolutionVolumes
        ...
        if options.axialEFOV && options.transaxialEFOV % Both transaxial and axial FOVs are extended.
            ...
            % Divide shift by multiresolution voxel size and adjust FOV size accordingly
            shiftX = round(options.multiResolutionShift(1) / dxM);
            shiftY = round(options.multiResolutionShift(2) / dyM);
            shiftZ = round(options.multiResolutionShift(3) / dzM);
        ...
        elseif options.transaxialEFOV && ~options.axialEFOV % Only the transaxial FOV is extended
            ...
            options.axial_fov = ([options.axialFOVOrig, FOVzM / 2 + shiftZ * dzM, FOVzM / 2 - shiftZ * dzM]);

            options.Nx = uint32([options.NxOrig, NxM, NxM]);

            options.Ny = uint32([options.NyOrig, NyM, NyM]);

            options.Nz = uint32([options.NzOrig, NzM2 / 2 + shiftZ, NzM2 / 2 - shiftZ]);
