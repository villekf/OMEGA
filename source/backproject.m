function [bp, varargout] = backproject(options, index, n_meas, rhs)
%BACKPROJECT Calculates the backprojection
% Example:
%   bp = backproject(options, index, n_meas, rhs)
% INPUTS:
...
if use_raw_data == false && (options.precompute_lor || options.reconstruction_method == 2 || options.reconstruction_method == 3 || options.reconstruction_method == 5)
...
elseif use_raw_data && (options.precompute_lor || options.reconstruction_method == 2  || options.reconstruction_method == 3  || options.reconstruction_method == 5)
...
%     options = double_to_single(options);
    ...
    [bp, norm] = OpenCL_matrixfree_multi_gpu( kernel_path, Ny, Nx, Nz, dx, dz, by, bx, bz, z_det, x, y, dy, yy(end), xx(end), ...
        ...
        uint32(options.projector_type), options.precompute_lor, int32(options.accuracy_factor), rhs, no_norm);
    toc
    varargout{1} = norm;
end