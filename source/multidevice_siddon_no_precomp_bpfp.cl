...
*
* Used by forward/backward projection. No precomputation.
*
* INPUTS:
...
void f_b_project_siddon(const uchar fp, const uint d_N, const uint d_Nx, const uint d_Ny, const uint d_Nz, const float d_dz, const float d_dx,
	...
	const uchar d_raw, const uint d_pRows, const uchar no_norm, const uint d_Nxy, const float dc_z, const ushort n_rays,
	const __global float* d_atten, const __global float* d_norm, __global float* d_Summ, __constant uint* d_pseudos, const __global float* d_x,
	...
	uint Np_n[5];
	bool pass[5];
	float d_rhs_local = 0.f, d_output_local = 0.f;
	if (fp == 0)
				...
				if (yd <= d_maxyy && yd >= d_by) {
					...
					if (fp == 1) {
						for (uint k = 0u; k < d_Nx; k++) {
							d_output_local += (d_rhs[apu + k] * d_dx);
						}
					}
				...
				if (xd <= d_maxxx && xd >= d_bx) {
					...
					if (fp == 1) {
						for (uint k = 0u; k < d_Ny; k++) {
							d_output_local += (d_rhs[apu + k * d_Nx] * d_dy);
						}
					}
				...
				if (tx0_a[lor] > 1e6f && ty0_a[lor] > 1e6f) {
					if (ty0_a[lor] > tx0_a[lor]) {
						...
						for (uint k = 0; k < d_Nx; k++) {
							...
							atomicAdd_g_f(&d_output[tempk + k], d_dx * temp * d_rhs_local);
						}
					}
					else {
						const int tempk = tempk_a[lor];
						for (uint k = 0; k < d_Ny; k++) {
							if (no_norm == 0u)
								atomicAdd_g_f(&d_Summ[tempk + k * d_Nx], (d_dy * temp));
							atomicAdd_g_f(&d_output[tempk + k * d_Nx], d_dy * temp * d_rhs_local);
						}
					}
				}
				else {
