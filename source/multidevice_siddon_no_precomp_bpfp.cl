...
void f_b_project_siddon(const uchar fp, const uint d_N, const uint d_Nx, const uint d_Ny, const uint d_Nz, const float d_dz, const float d_dx,
	...
	uint Np_n = 0u;
	uint local_ind;
	float local_ele, d_rhs_local = 0.f, d_output_local = 0.f;
	if (!fp)
	...
	if (fabs(z_diff) < 1e-6f) {
		...
		if (fabs(y_diff) < 1e-8f) {
			if (yd <= d_maxyy && yd >= d_by) {
				float templ_ijk = 0.f;
				uint tempk = 0u;
				perpendicular_elements(d_by, d_dy, d_Ny, yd, d_dx, d_Nx, d_atten, &templ_ijk, &tempk, d_attenuation_correction, z_loop, d_Ny, 1, d_normalization, local_norm);
				local_ele = templ_ijk;
				...
				if (fp)
					d_output[idx] = d_output_local + local_sc_ra;
			}
		}
			...
			if (xd <= d_maxxx && xd >= d_bx) {
				float templ_ijk = 0.f;
				uint tempk = 0u;
				perpendicular_elements(d_bx, d_dx, d_Nx, xd, d_dy, d_Ny, d_atten, &templ_ijk, &tempk, d_attenuation_correction, z_loop, 1, d_Nx, d_normalization, local_norm);
				local_ele = templ_ijk;
				for (uint ii = 0u; ii < Np; ii++) {
