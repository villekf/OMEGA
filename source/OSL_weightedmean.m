function x_weighted = OSL_weightedmean(options)
%OSL_FMH One Step Late algorithm with weigthed mean prior reconstruction.
%   Implements the OSL weigthed mean prior prior reconstruction on input 
%   PET data. See main_nongate.m for options-variables.
%
%   x_weighted = OSL_weightedmean(options) returns the OSL weigthed mean prior
...
x_weighted = zeros(options.Nx,options.Ny,options.Nz, options.Niter + 1);
x_weighted(:,:,:,1) = options.x0;
x_weighted = reshape(x_weighted, options.Nx*options.Ny*options.Nz, options.Niter + 1);
...
for ii = 1 : options.Niter
    ...
    for kk = 1 : options.subsets
        ...
        weighted_apu=((weighted_apu)./(full(sum(A,1)')+options.beta_weighted_osl*(weighted_apu-med)./(med+epps))).*((A'*(uu./(A*weighted_apu+epps))));
        if options.Nz==1
            pz_pad_weighted = padding(reshape(weighted_apu,options.Nx,options.Ny),[options.Ndx options.Ndy]);
        else
            pz_pad_weighted = padding(reshape(weighted_apu,options.Nx,options.Ny,options.Nz),[options.Ndx options.Ndy options.Ndz]);
        end
        tElapsed = toc(tStart);
        disp(['OSL FMH sub-iteration ' num2str(kk) ' took ' num2str(tElapsed) ' seconds'])
        disp(['OSL FMH sub-iteration ' num2str(kk) ' finished'])
    end
    x_weighted(:,ii+1) = weighted_apu;
end
x_weighted = reshape(x_weighted,options.Nx,options.Ny,options.Nz, options.Niter + 1);
