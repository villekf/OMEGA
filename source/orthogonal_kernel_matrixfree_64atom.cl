...
* d_Summ = sum(A,1) (sum of every row) and rhs = A*(y./(A'*x)), where A is 
* the system matrix, y the measurements and x the estimate/image.
*
* Used by implementation 2.
*
* This version uses 64-bit integer atomics (RHS and d_Summ need to be
...
void orth(__constant uchar* MethodList, const uchar d_raw, const float d_h, const uint d_Nx, const uint d_Ny, const uint d_Nz,
	...
	__constant float* x_center, __constant float* y_center, __constant float* z_center, __global ulong* d_Summ, const __global ushort* d_lor,
	...
	const __global float* d_OSEM, __global ulong* d_rhs_OSEM, const uchar no_norm, const ulong m_size, __local float* ax) {
...
__kernel __attribute__((vec_type_hint(float))) __attribute__((reqd_work_group_size(64, 1, 1)))
void orth_ml(const uchar d_raw, const uint d_Nx, const uint d_Ny, const uint d_Nz,
	const float d_dz, const float d_dx, const float d_dy, const float d_bz, const float d_bx, const float d_by, const float d_bzb, const float d_maxxx,
	const float d_maxyy, const float d_zmax, const float d_NSlices, const __global float* d_x, const __global float* d_y, const __global float* d_zdet,
	const uint d_size_x, const uint d_TotSinos, const uint d_attenuation_correction, const uint d_normalization, const uint d_randoms,
	const __global float* d_atten, const __global float* d_norm, const float d_epps, const uint d_N, __constant uint* d_pseudos, const uint d_pRows,
	...
	__constant float* y_center, __constant float* z_center, __global ulong* d_Summ, const __global ushort* d_lor, const __global uint* d_xyindex,
...
__kernel __attribute__((vec_type_hint(float))) __attribute__((reqd_work_group_size(64, 1, 1)))
void MRAMLA_prepass_orth(const uint d_det_per_ring, const uchar d_raw, __constant uint* d_pseudos, const uint d_pRows, const float d_h,
