...
if options.implementation == 1
    ...
    if options.osem || options.ecosem || options.attenuation_phase
        ...
        options.im_vectors.OSEM_apu = OSEM_im(options.im_vectors.OSEM_apu, A, options.epps, uu, Summ, SinD, options.is_transposed);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.mramla
        ...
        options.im_vectors.MRAMLA_apu = MBSREM(options.im_vectors.MRAMLA_apu, options.U, options.pj3, A, options.epps, uu, options.epsilon_mramla, options.lam_mbsrem, iter, SinD, options.randoms_correction, ...
    ...
    if options.rosem
        ...
        options.im_vectors.ROSEM_apu = ROSEM_subiter(options.im_vectors.ROSEM_apu, options.lam_rosem, iter, Summ, options.epps, A, uu, SinD, options.is_transposed);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.acosem
        ...
        [options.im_vectors.ACOSEM_apu, options.C_aco] = ACOSEM_im(options.im_vectors.ACOSEM_apu, A, options.epps, uu, options.C_aco, options.D, options.h, osa_iter, SinD, options.is_transposed);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.MRP && options.OSL_OSEM
        ...
        options.im_vectors.MRP_OSL_apu = OSL_OSEM(options.im_vectors.MRP_OSL_apu, Summ, options.beta_mrp_osem, med, options.epps, A, uu, SinD, options.is_transposed);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.MRP && options.MBSREM
        ...
        options.im_vectors.MRP_MBSREM_apu = MBSREM(options.im_vectors.MRP_MBSREM_apu, options.U, options.pj3, A, options.epps, uu, options.epsilon_mramla, options.lam_mbsrem, iter, SinD, options.randoms_correction, options.is_transposed, ...
    ...
    if options.MRP && options.BSREM
        ...
        if iter == 1 && options.ramla
            ...
            options.im_vectors.MRP_BSREM_apu = BSREM_subiter(options.im_vectors.MRP_BSREM_apu, options.lam, options.epps, iter, A, uu, SinD, options.is_transposed);
        end
    ...
    if options.quad && options.OSL_OSEM
        ...
        med = Quadratic_prior(options.im_vectors.Quad_OSL_apu, options.tr_offsets, options.weights, options.weights_quad, options.Nx, options.Ny, options.Nz, options.Ndx, options.Ndy, options.Ndz);
        options.im_vectors.Quad_OSL_apu = OSL_OSEM(options.im_vectors.Quad_OSL_apu, Summ, options.beta_quad_osem, med, options.epps, A, uu, SinD, options.is_transposed);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.quad && options.MBSREM
        ...
        med = Quadratic_prior(options.im_vectors.Quad_MBSREM_apu, options.tr_offsets, options.weights, options.weights_quad, options.Nx, options.Ny, options.Nz, options.Ndx, options.Ndy, options.Ndz);
        options.im_vectors.Quad_MBSREM_apu = MBSREM(options.im_vectors.Quad_MBSREM_apu, options.U, options.pj3, A, options.epps, uu, options.epsilon_mramla, options.lam_mbsrem, iter, SinD, options.randoms_correction, options.is_transposed, ...
    ...
    if options.quad && options.RBI_MAP
        ...
        med = Quadratic_prior(options.im_vectors.Quad_RBI_apu, options.tr_offsets, options.weights, options.weights_quad, options.Nx, options.Ny, options.Nz, options.Ndx, options.Ndy, options.Ndz);
    ...
    if options.quad && any(options.COSEM_MAP)
        ...
        med = Quadratic_prior(options.im_vectors.Quad_COSEM_apu, options.tr_offsets, options.weights, options.weights_quad, options.Nx, options.Ny, options.Nz, options.Ndx, options.Ndy, options.Ndz);
        if options.COSEM_MAP == 1
    ...
    if options.L && options.OSL_OSEM
        ...
        med = L_filter(options.im_vectors.L_OSL_apu, options.tr_offsets, options.a_L, options.Nx, options.Ny, options.Nz, options.Ndx, options.Ndy, options.Ndz, options.epps, options.med_no_norm);
        options.im_vectors.L_OSL_apu = OSL_OSEM(options.im_vectors.L_OSL_apu, Summ, options.beta_L_osem, med, options.epps, A, uu, SinD, options.is_transposed);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.L && options.MBSREM
        ...
        med = L_filter(options.im_vectors.L_MBSREM_apu, options.tr_offsets, options.a_L, options.Nx, options.Ny, options.Nz, options.Ndx, options.Ndy, options.Ndz, options.epps, options.med_no_norm);
        options.im_vectors.L_MBSREM_apu = MBSREM(options.im_vectors.L_MBSREM_apu, options.U, options.pj3, A, options.epps, uu, options.epsilon_mramla, options.lam_mbsrem, iter, SinD, options.randoms_correction, options.is_transposed, ...
    ...
    if options.L && options.RBI_MAP
        ...
        med = L_filter(options.im_vectors.L_RBI_apu, options.tr_offsets, options.a_L, options.Nx, options.Ny, options.Nz, options.Ndx, options.Ndy, options.Ndz, options.epps, options.med_no_norm);
    ...
    if options.L && any(options.COSEM_MAP)
        ...
        med = L_filter(options.im_vectors.L_COSEM_apu, options.tr_offsets, options.a_L, options.Nx, options.Ny, options.Nz, options.Ndx, options.Ndy, options.Ndz, options.epps, options.med_no_norm);
        if options.COSEM_MAP == 1
    ...
    if options.FMH && options.OSL_OSEM
        ...
        med = FMH(options.im_vectors.FMH_OSL_apu, options.tr_offsets, options.fmh_weights, options.weights, options.Nx, options.Ny, options.Nz, N, options.Ndx, options.Ndy, options.Ndz, options.epps, ...
            options.med_no_norm);
        options.im_vectors.FMH_OSL_apu = OSL_OSEM(options.im_vectors.FMH_OSL_apu, Summ, options.beta_fmh_osem, med, options.epps, A, uu, SinD, options.is_transposed);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.FMH && options.MBSREM
        ...
        med = FMH(options.im_vectors.FMH_MBSREM_apu, options.tr_offsets, options.fmh_weights, options.weights, options.Nx, options.Ny, options.Nz, N, options.Ndx, options.Ndy, options.Ndz, options.epps, ...
            options.med_no_norm);
        options.im_vectors.FMH_MBSREM_apu = MBSREM(options.im_vectors.FMH_MBSREM_apu, options.U, options.pj3, A, options.epps, uu, options.epsilon_mramla, options.lam_mbsrem, iter, SinD, options.randoms_correction, options.is_transposed, ...
    ...
    if options.FMH && options.RBI_MAP
        ...
        med = FMH(options.im_vectors.FMH_RBI_apu, options.tr_offsets, options.fmh_weights, options.weights, options.Nx, options.Ny, options.Nz, N, options.Ndx, options.Ndy, options.Ndz, options.epps, ...
            options.med_no_norm);
    ...
    if options.FMH && any(options.COSEM_MAP)
        ...
        med = FMH(options.im_vectors.FMH_COSEM_apu, options.tr_offsets, options.fmh_weights, options.weights, options.Nx, options.Ny, options.Nz, N, options.Ndx, options.Ndy, options.Ndz, options.epps, ...
            options.med_no_norm);
        if options.COSEM_MAP == 1
    ...
    if options.weighted_mean && options.OSL_OSEM
        ...
        options.im_vectors.Weighted_OSL_apu = OSL_OSEM(options.im_vectors.Weighted_OSL_apu, Summ, options.beta_weighted_osem, med, options.epps, A, uu, SinD, ...
            options.is_transposed);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.weighted_mean && options.MBSREM
        ...
        med = Weighted_mean(options.im_vectors.Weighted_MBSREM_apu, options.tr_offsets, options.weighted_weights, options.Nx, options.Ny, options.Nz, options.Ndx, options.Ndy, options.Ndz, ...
            options.mean_type, options.epps, options.w_sum, options.med_no_norm);
        options.im_vectors.Weighted_MBSREM_apu = MBSREM(options.im_vectors.Weighted_MBSREM_apu, options.U, options.pj3, A, options.epps, uu, options.epsilon_mramla, ...
            options.lam_mbsrem, iter, SinD, options.randoms_correction, options.is_transposed, options.beta_weighted_mbsrem, med);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.TV && options.OSL_OSEM
        ...
        grad = TVpriorFinal(options.im_vectors.TV_OSL_apu, TVdata, options.Nx, options.Ny, options.Nz, options.TV_use_anatomical, options, options.TVtype, ...
            options.tr_offsets);
        options.im_vectors.TV_OSL_apu = OSL_OSEM(options.im_vectors.TV_OSL_apu, Summ, options.beta_TV_osem, grad, options.epps, A, uu, SinD, options.is_transposed);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.TV && options.MBSREM
        ...
        options.im_vectors.TV_MBSREM_apu = MBSREM(options.im_vectors.TV_MBSREM_apu, options.U, options.pj3, A, options.epps, uu, options.epsilon_mramla, options.lam_mbsrem, ...
            iter, SinD, options.randoms_correction, options.is_transposed, options.beta_TV_mbsrem, grad);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.AD && options.OSL_OSEM
        ...
        if osa_iter > 1
            ...
            options.im_vectors.AD_OSL_apu = OSL_OSEM(options.im_vectors.AD_OSL_apu, Summ, options.beta_ad_osem, med, options.epps, A, uu, SinD, options.is_transposed);
        else
            options.im_vectors.AD_OSL_apu = OSEM_im(options.im_vectors.AD_OSL_apu, A, options.epps, uu, Summ);
        end
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.AD && options.MBSREM
        ...
        options.im_vectors.AD_MBSREM_apu = MBSREM(options.im_vectors.AD_MBSREM_apu, options.U, options.pj3, A, options.epps, uu, options.epsilon_mramla, options.lam_mbsrem, ...
            iter, SinD, options.randoms_correction, options.is_transposed, options.beta_ad_mbsrem, med);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.APLS && options.OSL_OSEM
        ...
        options.im_vectors.APLS_OSL_apu = OSL_OSEM(options.im_vectors.APLS_OSL_apu, Summ, options.beta_APLS_osem, grad, options.epps, A, uu, SinD, options.is_transposed);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.APLS && options.MBSREM
        ...
        options.im_vectors.APLS_MBSREM_apu = MBSREM(options.im_vectors.APLS_MBSREM_apu, options.U, options.pj3, A, options.epps, uu, options.epsilon_mramla, options.lam_mbsrem, ...
            iter, SinD, options.randoms_correction, options.is_transposed, options.beta_APLS_mbsrem, grad);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.TGV && options.OSL_OSEM
        ...
        options.im_vectors.TGV_OSL_apu = OSL_OSEM(options.im_vectors.TGV_OSL_apu, Summ, options.beta_TGV_osem, grad, options.epps, A, uu, SinD, options.is_transposed);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.TGV && options.MBSREM
        ...
        options.im_vectors.TGV_MBSREM_apu = MBSREM(options.im_vectors.TGV_MBSREM_apu, options.U, options.pj3, A, options.epps, uu, options.epsilon_mramla, options.lam_mbsrem, ...
            iter, SinD, options.randoms_correction, options.is_transposed, options.beta_TGV_mbsrem, grad);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.NLM && options.OSL_OSEM
        ...
        options.im_vectors.NLM_OSL_apu = OSL_OSEM(options.im_vectors.NLM_OSL_apu, Summ, options.beta_NLM_osem, med, options.epps, A, uu, SinD, options.is_transposed);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.NLM && options.MBSREM
        ...
        options.im_vectors.NLM_MBSREM_apu = MBSREM(options.im_vectors.NLM_MBSREM_apu, options.U, options.pj3, A, options.epps, uu, options.epsilon_mramla, options.lam_mbsrem, ...
            iter, SinD, options.randoms_correction, options.is_transposed, options.beta_NLM_mbsrem, med);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.OSL_OSEM && options.custom
        ...
        options.im_vectors.custom_OSL_apu = OSL_OSEM(options.im_vectors.custom_OSL_apu, Summ, options.beta_custom_osem, options.grad_OSEM, options.epps, A, uu, SinD, options.is_transposed);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.MBSREM && options.custom
        ...
        options.im_vectors.custom_MBSREM_apu = MBSREM(options.im_vectors.custom_MBSREM_apu, options.U, options.pj3, A, options.epps, uu, options.epsilon_mramla, options.lam_mbsrem, iter, SinD, options.randoms_correction, options.is_transposed, ptions.beta_custom_mbsrem, options.grad_MBSREM);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.ROSEM_MAP && options.custom
        ...
        if iter == 1 && options.rosem
            ...
            options.im_vectors.custom_ROSEM_apu = ROSEM_subiter(options.im_vectors.custom_ROSEM_apu, options.lam_rosem, iter, Summ, options.epps, A, uu, SinD, options.is_transposed);
        end
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if options.RBI_MAP && options.custom
        ...
        options.im_vectors.custom_RBI_apu = RBI_subiter(options.im_vectors.custom_RBI_apu, A, uu, options.epps, Summ, options.D, SinD, options.is_transposed, options.beta_custom_rbi, options.grad_RBI);
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if any(options.COSEM_MAP) && options.custom
        ...
        if options.COSEM_MAP == 1
            [options.im_vectors.custom_COSEM_apu, options.C_osl] = COSEM_OSL(options.im_vectors.custom_COSEM_apu, options.D, options.beta_custom_cosem, options.grad_COSEM, A, uu, options.epps, options.C_aco, options.h, options.COSEM_MAP, osa_iter, SinD, options.is_transposed);
        else
            [options.im_vectors.custom_COSEM_apu, options.C_osl] = COSEM_OSL(options.im_vectors.custom_COSEM_apu, options.D, options.beta_custom_cosem, options.grad_COSEM, A, uu, options.epps, options.C_co, 0, options.COSEM_MAP, osa_iter, SinD, options.is_transposed);
        end
        if options.verbose
            tElapsed = toc(tStart);
    ...
    if isfield(options, 'grad_COSEM') && any(options.COSEM_MAP)
    ...
    if options.projector_type == 1
        kernel_file = 'siddon_kernel_matrixfree_GPU.cl';
        ...
        header_directory = strrep(kernel_path,'siddon_kernel_matrixfree_GPU','');
    elseif options.projector_type == 2
        ...
        header_directory = strrep(kernel_path,'orthogonal_kernel_matrixfree','');
    end
    ...
    n_rekos = uint32(sum(options.rekot(~contains(joku,'MLEM'))));
    n_rekos_mlem = uint32(sum(options.rekot(contains(joku,'MLEM'))));
    reko_type = zeros(n_rekos,1,'uint8');
    joku = joku(options.rekot & ~contains(joku,'MLEM'));
    ...
    if options.ecosem
        if options.cosem && options.osem
            reko_type(contains(joku,'ECOSEM')) = [];
    ...
    tic
    [pz] = OpenCL_matrixfree( kernel_path, options.Ny, options.Nx, options.Nz, options.dx, options.dz, options.by, options.bx, options.bz, options.z_det, options.x, options.y, options.dy, options.yy(end), options.xx(end) , options.NSinos, single(options.NSlices), options.size_x, options.zmax, options.NSinos, ...
        options.verbose, options.LL, options.pseudot, options.det_per_ring, uint32(options.use_device), uint8(options.use_raw_data), filename, uint32(0), options.force_build, header_directory, options.vaimennus, options.normalization, ...
        options.pituus, uint32(options.attenuation_correction), uint32(options.normalization_correction), uint32(options.Niter), uint32(options.subsets), uint8(rekot), single(options.epps), options.lor_a, options.xy_index, ...
        options.z_index, any(n_rekos), tube_width_xy, crystal_size_z, options.x_center, options.y_center, options.z_center, options.SinDelayed, randoms, uint32(options.projector_type), options.precompute_lor, ...
        int32(options.accuracy_factor), n_rays, dc_z, options, options.SinM, uint32(options.partitions), logical(options.use_64bit_atomics), n_rekos, n_rekos_mlem, reko_type);
    toc
    
    options.im_vectors = transfer_im_vectors(options.im_vectors, pz, options);
    %     if (options.cosem || options.ecosem)
