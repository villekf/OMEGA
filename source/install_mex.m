...
if exist('OCTAVE_VERSION','builtin') == 0
    ...
    if implementation == 0 || implementation == 1 || implementation == 4
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Inveon support %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        try
            mex(compiler, '-largeArrayDims', '-outdir', folder, [folder '/inveon_list2matlab.cpp'])
        ...
        if implementation == 0 || implementation == 2
            try
                ...
                mex(compiler, complexFlag, '-outdir', folder, compflags, cxxflags, '-lafopencl', cxxlib, ['-L' af_path '/lib64'], ['-L"' af_path '/lib"'], ...
                    ...
                    ['-I' af_path_include], ['-I"' cuda_path '/include"'], ['-I"' opencl_include_path '"'], '-I/opt/AMDAPPSDK-3.0/include', ...
                    [folder '/OpenCL_matrixfree.cpp'], [folder '/functions.cpp'], [folder '/reconstruction_AF_matrixfree.cpp'], ...
                    [folder '/opencl_error.cpp'], ...
                    [folder '/compute_OS_estimates_iter.cpp'], [folder '/compute_OS_estimates_subiter.cpp'], [folder '/mexFunktio.cpp'], [folder '/computeForwardProject.cpp'])
                disp('Implementation 2 built')
            catch ME
                if verbose
                    warning(['Unable to build OpenCL files for implementation 2! If you do not need implementation 2 (matrix free OpenCL with ArrayFire) ignore this warning. '...
    ...
    [~, sys] = mkoctfile('--mex','-DMATLAB', '-DCT', ['-I ' folder], OMPlib,...
        [folder '/projector_mex.cpp'], [folder '/projector_functions.cpp'], [folder '/improved_siddon_precomputed.cpp'], ...
        [folder '/orth_siddon_precomputed.cpp'], [folder '/sequential_improved_siddon_openmp.cpp'], [folder '/sequential_improved_siddon_no_precompute_openmp.cpp'], ...
        [folder '/improved_siddon_no_precompute.cpp'], [folder '/original_siddon_function.cpp'], [folder '/improved_Siddon_algorithm_discard.cpp'],...
        [folder '/volume_projector_functions.cpp'], [folder '/vol_siddon_precomputed.cpp']);
    ...
    if sys == 0
        ...
        if verbose
            mkoctfile('--mex','-DMATLAB', ['-I ' folder], [folder '/projector_mex.cpp'], [folder '/projector_functions.cpp'], ...
                [folder '/improved_siddon_precomputed.cpp'], [folder '/orth_siddon_precomputed.cpp'], [folder '/sequential_improved_siddon_openmp.cpp'], ...
                [folder '/sequential_improved_siddon_no_precompute_openmp.cpp'], [folder '/improved_siddon_no_precompute.cpp'], ...
            ...
            mkoctfile('--mex','-DMATLAB', ['-I ' folder], '"--std=c++11"', [folder '/projector_mex.cpp'], [folder '/projector_functions.cpp'], ...
                [folder '/improved_siddon_precomputed.cpp'], [folder '/orth_siddon_precomputed.cpp'], [folder '/sequential_improved_siddon_openmp.cpp'], ...
                [folder '/sequential_improved_siddon_no_precompute_openmp.cpp'], [folder '/improved_siddon_no_precompute.cpp'], ...
                [folder '/original_siddon_function.cpp'], [folder '/improved_Siddon_algorithm_discard.cpp'], ...
                [folder '/volume_projector_functions.cpp'], [folder '/vol_siddon_precomputed.cpp'])
    ...
    if (ispc)
        ...
        if isempty(opencl_include_path)
            ...
            if isempty(af_path)
                ...
                %%%%%%%%%%%%%%%%%%%%%% Implementation 2 %%%%%%%%%%%%%%%%%%%%%%%
                [~, sys] = mkoctfile('--mex', '-DOPENCL', '-Wno-ignored-attributes', '-lafopencl', '-lOpenCL', ...
                    ...
                    [folder '/precomp.cpp'], [folder '/find_lors.cpp'], [folder '/AF_opencl_functions.cpp'], [folder '/compute_ML_estimates.cpp'], ...
                ...
                if sys ~= 0
                    charArray = mkoctfile('--mex', '-v', '-DOPENCL', '-Wno-ignored-attributes', '-lafopencl', '-lOpenCL', ['-L' af_path '\lib64'], ['-L' af_path '\lib'],['-L' opencl_lib_path], ...
                        ['-I' folder], ['-I' opencl_include_path], ['-I' af_path_include], [folder '/OpenCL_matrixfree.cpp'],[folder '/functions.cpp'],[folder '/opencl_error.cpp'], ...
                        [folder '/reconstruction_AF_matrixfree.cpp'], [folder '/precomp.cpp'], [folder '/find_lors.cpp'], [folder '/AF_opencl_functions.cpp'], ...
                        [folder '/compute_ML_estimates.cpp'], [folder '/compute_OS_estimates_iter.cpp'], [folder '/compute_OS_estimates_subiter.cpp'], [folder '/mexFunktio.cpp']);
                    sys = makeOCT(charArray);
                end
                syst = syst + sys;
