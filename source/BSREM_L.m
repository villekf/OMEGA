function x_L = BSREM_L(options)
%BSREM_L Block Sequential Regularized Expectation Maximization algorithm
%with L-filter prior reconstruction.
%   Implements the BSREM L-filter prior reconstruction on input PET data.
%   See main_nongate.m for options-variables.
%
%   x_L = BSREM_L(options) returns the BSREM L-filter reconstructions
...
for ii = 1 : options.Niter
    ...
    for kk = 1 : options.subsets
        ...
        tStart = tic;
        L_apu = L_apu + lam(ii).*L_apu./(full(sum(A,1)')+epps).*(A'*(uu./(A*L_apu+epps))-full(sum(A,1)'));
        tElapsed = toc(tStart);
        disp(['BSREM L-filter sub-iteration ' num2str(kk) ' took ' num2str(tElapsed) ' seconds'])
        disp(['BSREM L-filter sub-iteration ' num2str(kk) ' finished'])
    end
    tStart = tic;
    if options.Nz==1
        pz_pad_L_bsrem = padding(reshape(L_apu,options.Nx,options.Ny),[options.Ndx options.Ndy]);
        med = sort(pz_pad_L_bsrem(tr_offsets),2)*a_L;
    else
        pz_pad_L_bsrem = padding(reshape(L_apu,options.Nx,options.Ny,options.Nz),[options.Ndx options.Ndy options.Ndz]);
        med = sort(pz_pad_L_bsrem(tr_offsets),2)*a_L;
    end
    x_L(:,ii+1) = L_apu - options.beta_L_bsrem*lam(ii).*(L_apu-med(:)).*L_apu./med(:);
    x_L(x_L(:,ii+1)<0,ii+1) = epps;
    tElapsed = toc(tStart);
    disp(['BSREM L-filter iteration ' num2str(ii) ' took ' num2str(tElapsed) ' seconds'])
end
x_L = reshape(x_L,options.Nx,options.Ny,options.Nz, options.Niter + 1);
