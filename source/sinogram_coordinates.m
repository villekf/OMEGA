...
% j = mod(j + options.offangle, det_w_pseudo/2);
% b = (j >= options.offangle);
% i(b) = -i(b);
% temppi(b) = ~temppi(b);
% temppi2(b) = ~temppi2(b);
...
% % include mashing (if applicable)
% if (det_w_pseudo/2/Nang) > 1
%     %     apux = zeros(size(x,1)/(det_w_pseudo/2/Nang),size(x,2));
%     %     apux2 = zeros(size(x2,1)/(det_w_pseudo/2/Nang),size(x2,2));
%     %     apuy = zeros(size(y,1)/(det_w_pseudo/2/Nang),size(y,2));
%     %     apuy2 = zeros(size(y2,1)/(det_w_pseudo/2/Nang),size(y2,2));
%     for kk = 1 : size(x,1)
%         for ll = 1 : size(x,2)
%             if isnan(x(kk,ll))
%                 if abs(diff([x(kk-1,ll),x(kk+1,ll)])) > threshold
%                     if round(x(kk,ll+1),-2) == round(x(kk+1,ll),-2)
%                         if x(kk+1,ll-1) == x(kk+1,ll)
%                             x(kk,ll) = x(kk,ll-1);
%                         elseif x(kk+1,ll+1) == x(kk+1,ll)
%                             x(kk,ll) = x(kk,ll+1);
%                         else
%                             x(kk,ll) = (diff(x(kk+1:kk+2,ll)))+x(kk+1,ll);
%                         end
%                         if x2(kk+1,ll-1) == x2(kk+1,ll)
%                             x2(kk,ll) = x2(kk,ll-1);
%                         elseif x2(kk+1,ll+1) == x2(kk+1,ll)
%                             x2(kk,ll) = x2(kk,ll+1);
%                         else
%                             x2(kk,ll) = (diff(x2(kk+2:-1:kk+1,ll)))+x2(kk+1,ll);
%                         end
%                         if y(kk+1,ll-1) == y(kk+1,ll)
%                             y(kk,ll) = y(kk,ll-1);
%                         elseif y(kk+1,ll+1) == y(kk+1,ll)
%                             y(kk,ll) = y(kk,ll+1);
%                         else
%                             y(kk,ll) = (diff(y(kk+2:-1:kk+1,ll)))+y(kk+1,ll);
%                         end
%                         if y2(kk+1,ll-1) == y2(kk+1,ll)
%                             y2(kk,ll) = y2(kk,ll-1);
%                         elseif y2(kk+1,ll+1) == y2(kk+1,ll)
%                             y2(kk,ll) = y2(kk,ll+1);
%                         else
%                             y2(kk,ll) = (diff(y2(kk+2:-1:kk+1,ll)))+y2(kk+1,ll);
%                         end
%                     else
%                         if x(kk-1,ll+1) == x(kk-1,ll)
%                             x(kk,ll) = x(kk,ll+1);
...
%                             x(kk,ll) = x(kk,ll+1);
%                         else
%                             x(kk,ll) = (diff(x(kk-2:kk-1,ll)))+x(kk-1,ll);
%                         end
%                         if x2(kk-1,ll+1) == x2(kk-1,ll)
%                             x2(kk,ll) = x2(kk,ll+1);
%                         elseif x2(kk+1,ll+1) == x2(kk+1,ll)
%                             x2(kk,ll) = x2(kk,ll+1);
%                         else
%                             x2(kk,ll) = (diff(x2(kk-2:kk-1,ll)))+x2(kk-1,ll);
%                         end
%                         if y(kk-1,ll+1) == y(kk-1,ll)
%                             y(kk,ll) = y(kk,ll+1);
%                         elseif y(kk+1,ll+1) == y(kk+1,ll)
%                             y(kk,ll) = y(kk,ll+1);
%                         else
%                             y(kk,ll) = (diff(y(kk-2:kk-1,ll)))+y(kk-1,ll);
%                         end
%                         if y2(kk-1,ll+1) == y2(kk-1,ll)
%                             y2(kk,ll) = y2(kk,ll+1);
%                         elseif y2(kk+1,ll+1) == y2(kk+1,ll)
%                             y2(kk,ll) = y2(kk,ll+1);
%                         else
%                             y2(kk,ll) = (diff(y2(kk-2:kk-1,ll)))+y2(kk-1,ll);
%                         end
%                     end
%                 else
%                     x(kk,ll) = mean([x(kk+1,ll) x(kk-1,ll)]);
...
% % LOR pair (includes possible pseudo detectors)
% jh = int32(1);
% L = zeros(sum(1:det_w_pseudo),2,'int32');
% for kk = int32(1) : (det_w_pseudo)
%     L(jh:(jh + (det_w_pseudo) - 1),:) = [repelem((kk), det_w_pseudo)', ((1):det_w_pseudo)'];
%     jh = jh + (det_w_pseudo);
% end
%
% L = L - 1;
...
% ii=abs(xa-ya-det_w_pseudo/2);
% for kk = 1 : length(ii)
%     if (ya(kk)<jj(kk)) || (b(kk)<xa(kk))
%         ii(kk)=-ii(kk);
%     end
% end
%
% jj = mod(jj + options.offangle, det_w_pseudo/2);
% b = (jj >= options.offangle);
% ii(b) = -ii(b);
%
% if mod(Ndist,2) == 0
%     joku12_2 = (ii <= (Ndist/2 + min(0,options.ndist_side)) & ii >= (-Ndist/2 + max(0,options.ndist_side)));
% else
%     joku12_2 = (ii <= Ndist/2 & ii >= (-Ndist/2));
% end
%
%
% ii = ii(joku12_2);
...
% %%
%
% % Form the actual sinogram coordinates (without mashing)
% x = accumarray([jj ii], xx1, [Nang*(det_w_pseudo/2/Nang) Ndist],@mean,NaN);
% y = accumarray([jj ii], yy1, [Nang*(det_w_pseudo/2/Nang) Ndist],@mean,NaN);
%
%
% x2 = accumarray([jj ii], xx2, [Nang*(det_w_pseudo/2/Nang) Ndist],@mean,NaN);
% y2 = accumarray([jj ii], yy2, [Nang*(det_w_pseudo/2/Nang) Ndist],@mean,NaN);
%
%
% threshold = max([mean(mean(abs(diff(x)))), mean(mean(abs(diff(x2)))), mean(mean(abs(diff(y)))), mean(mean(abs(diff(y2))))]);
%
% % include mashing (if applicable)
% if (det_w_pseudo/2/Nang) > 1
%     apux = zeros(size(x,1)/(det_w_pseudo/2/Nang),size(x,2));
...
%
%         for ll = 1 : size(x,2)
%             if abs(diff(x(kk:kk+1,ll))) > threshold
%                 apux(lk,ll) = mean([x(kk:kk,ll),x2(kk+1:kk+1,ll)]);
%                 apuy(lk,ll) = mean([y(kk:kk,ll),y2(kk+1:kk+1,ll)]);
%                 apux2(lk,ll) = mean([x(kk+1:kk+1,ll),x2(kk:kk,ll)]);
%                 apuy2(lk,ll) = mean([y(kk+1:kk+1,ll),y2(kk:kk,ll)]);
%             else
%                 apux(lk,ll) = mean(x(kk:kk+1,ll));
%                 apuy(lk,ll) = mean(y(kk:kk+1,ll));
%                 apux2(lk,ll) = mean(x2(kk:kk+1,ll));
%                 apuy2(lk,ll) = mean(y2(kk:kk+1,ll));
%             end
%         end
%         lk = lk + 1;
%     end
