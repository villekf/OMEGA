...
void computeOSEstimatesIter(AF_im_vectors& vec, Weighting& w_vec, const RecMethods& MethodList, const uint32_t im_dim, const float epps,
	...
	if (MethodList.Quad) {
		...
		if (MethodList.BSREM) {
			...
			vec.Quad_BSREM(span, it) = MAP(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.lambda_BSREM[iter], beta.Quad_BSREM, dU, epps);
			yy += im_dim;
		}
		if (MethodList.MBSREM) {
		...
		if (MethodList.ROSEMMAP) {
			array dU = Quadratic_prior(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.Ndx, w_vec.Ndy, w_vec.Ndz, Nx, Ny, Nz, w_vec.inffi,
				w_vec.tr_offsets, w_vec.weights_quad, im_dim);
			vec.Quad_ROSEM(span, it) = MAP(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.lambda_ROSEM[iter], beta.Quad_ROSEM, dU, epps);
			yy += im_dim;
		}
		if (MethodList.RBIOSL) {
	...
	if (MethodList.Huber) {
		...
		if (MethodList.BSREM) {
			...
			vec.Huber_BSREM(span, it) = MAP(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.lambda_BSREM[iter], beta.Huber_BSREM, dU, epps);
			yy += im_dim;
		}
		if (MethodList.MBSREM) {
		...
		if (MethodList.ROSEMMAP) {
			array dU = Huber_prior(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.Ndx, w_vec.Ndy, w_vec.Ndz, Nx, Ny, Nz, w_vec.inffi,
				w_vec.tr_offsets, w_vec.weights_huber, im_dim, w_vec.huber_delta);
			vec.Huber_ROSEM(span, it) = MAP(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.lambda_ROSEM[iter], beta.Huber_ROSEM, dU, epps);
			yy += im_dim;
		}
		if (MethodList.RBIOSL) {
	...
	if (MethodList.L) {
		...
		if (MethodList.BSREM) {
			...
			vec.L_BSREM(span, it) = MAP(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.lambda_BSREM[iter], beta.L_BSREM, dU, epps);
			yy += im_dim;
		}
		if (MethodList.MBSREM) {
		...
		if (MethodList.ROSEMMAP) {
			array dU = L_filter(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.Ndx, w_vec.Ndy, w_vec.Ndz, Nx, Ny, Nz, epps, w_vec.tr_offsets, w_vec.a_L,
				w_vec.med_no_norm, im_dim);
			vec.L_ROSEM(span, it) = MAP(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.lambda_ROSEM[iter], beta.L_ROSEM, dU, epps);
			yy += im_dim;
		}
		if (MethodList.RBIOSL) {
	...
	if (MethodList.FMH) {
		...
		if (MethodList.BSREM) {
			...
			vec.FMH_BSREM(span, it) = MAP(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.lambda_BSREM[iter], beta.FMH_BSREM, dU, epps);
			yy += im_dim;
		}
		if (MethodList.MBSREM) {
		...
		if (MethodList.ROSEMMAP) {
			array dU = FMH(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.Ndx, w_vec.Ndy, w_vec.Ndz, Nx, Ny, Nz, epps, w_vec.inffi, w_vec.tr_offsets,
				w_vec.fmh_weights, w_vec.med_no_norm, w_vec.alku_fmh, im_dim);
			vec.FMH_ROSEM(span, it) = MAP(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.lambda_ROSEM[iter], beta.FMH_ROSEM, dU, epps);
			yy += im_dim;
		}
		if (MethodList.RBIOSL) {
	...
	if (MethodList.WeightedMean) {
		...
		if (MethodList.BSREM) {
			...
			vec.Weighted_BSREM(span, it) = MAP(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.lambda_BSREM[iter], beta.Weighted_BSREM, dU,
				epps);
			yy += im_dim;
		}
		if (MethodList.MBSREM) {
		...
		if (MethodList.ROSEMMAP) {
			array dU = Weighted_mean(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.Ndx, w_vec.Ndy, w_vec.Ndz, Nx, Ny, Nz, epps, w_vec.weighted_weights, 
				w_vec.med_no_norm, im_dim, w_vec.mean_type, w_vec.w_sum);
			vec.Weighted_ROSEM(span, it) = MAP(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.lambda_ROSEM[iter], beta.Weighted_ROSEM, dU,
				epps);
			yy += im_dim;
		}
		if (MethodList.RBIOSL) {
	...
	if (MethodList.TV) {
		...
		if (MethodList.BSREM) {
			...
			vec.TV_BSREM(span, it) = MAP(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.lambda_BSREM[iter], beta.TV_BSREM, dU, epps);
			yy += im_dim;
		}
		if (MethodList.MBSREM) {
		...
		if (MethodList.ROSEMMAP) {
			array dU = TVprior(Nx, Ny, Nz, data, vec.im_os(seq(yy, yy + im_dim - 1u)), epps, data.TVtype, w_vec, w_vec.tr_offsets);
			vec.TV_ROSEM(span, it) = MAP(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.lambda_ROSEM[iter], beta.TV_ROSEM, dU, epps);
			yy += im_dim;
		}
		if (MethodList.RBIOSL) {
	...
	if (MethodList.AD) {
		...
		if (MethodList.BSREM) {
			...
			vec.AD_BSREM(span, it) = MAP(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.lambda_BSREM[iter], beta.AD_BSREM, dU, epps);
			yy += im_dim;
		}
		if (MethodList.MBSREM) {
		...
		if (MethodList.ROSEMMAP) {
			array dU = AD(vec.im_os(seq(yy, yy + im_dim - 1u)), Nx, Ny, Nz, epps, w_vec.TimeStepAD, w_vec.KAD, w_vec.NiterAD, w_vec.FluxType,
				w_vec.DiffusionType, w_vec.med_no_norm);
			vec.AD_ROSEM(span, it) = MAP(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.lambda_ROSEM[iter], beta.AD_ROSEM, dU, epps);
			yy += im_dim;
		}
		if (MethodList.RBIOSL) {
	...
	if (MethodList.TGV) {
		...
		if (MethodList.BSREM) {
			...
			vec.TGV_BSREM(span, it) = MAP(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.lambda_BSREM[iter], beta.TGV_BSREM, dU, epps);
			yy += im_dim;
		}
		if (MethodList.MBSREM) {
		...
		if (MethodList.ROSEMMAP) {
			array dU = TGV(vec.im_os(seq(yy, yy + im_dim - 1u)), Nx, Ny, Nz, data.NiterTGV, data.TGVAlpha, data.TGVBeta);
			vec.TGV_ROSEM(span, it) = MAP(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.lambda_ROSEM[iter], beta.TGV_ROSEM, dU, epps);
			yy += im_dim;
		}
		if (MethodList.RBIOSL) {
	...
	if (MethodList.NLM) {
		...
		if (MethodList.BSREM) {
			...
			vec.NLM_BSREM(span, it) = MAP(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.lambda_BSREM[iter], beta.NLM_BSREM, dU, epps);
			yy += im_dim;
		}
		if (MethodList.MBSREM) {
		...
		if (MethodList.ROSEMMAP) {
			array dU = NLM(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec, epps, Nx, Ny, Nz, OpenCLStruct);
			vec.NLM_ROSEM(span, it) = MAP(vec.im_os(seq(yy, yy + im_dim - 1u)), w_vec.lambda_ROSEM[iter], beta.NLM_ROSEM, dU, epps);
			yy += im_dim;
		}
		if (MethodList.RBIOSL) {
