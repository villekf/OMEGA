...
#include "siddon_kernel_matrixfree_GPU.cl"
#define TYPE1 0
...
void siddon_noprecomp(__constant uchar* MethodList, const uchar d_raw, const float d_h, const uint d_Nx, const uint d_Ny, const uint d_Nz,
	...
	const uint d_Nxy, const uint d_det_per_ring, const uint n_rekos, const float dc_z, const ushort n_rays, __global float* d_Summ, const __global ushort* d_lor, 
	...
	const __global float* d_sc_ra, const __global float* d_OSEM, __global float* d_rhs_OSEM,
				...
				if (ty0_a[lor] > tx0_a[lor]) {
					...
					if (local_sino > 0.f) {
						for (uint k = 0; k < d_Nx; k++) {
							if (no_norm == 0u)
								atomicAdd_g_f(&d_Summ[tempk + k], local_ele);
					...
					if (local_sino > 0.f) {
						for (uint k = 0; k < d_Ny; k++) {
							if (no_norm == 0u)
								atomicAdd_g_f(&d_Summ[tempk + k * d_Nx], local_ele);
