...
typedef struct Weighting_ {
	...
	std::vector<af::array> dU, M, preRef, D, gradF, RDPref;
    std::vector<std::vector<af::array>> dP;
	af::array gaussianNLM, gFilter, filter, filterIm, Ffilter;
	af_flux_function FluxType;
	af_diffusion_eq DiffusionType;
#endif
	TVdata data;
	float *thetaCP = nullptr, *sigmaCP = nullptr, *sigma2CP = nullptr, *lambda = nullptr, * alphaM = nullptr, * sigma_PKMA = nullptr, * uv = nullptr, * angles = nullptr, * NLM_ref = nullptr, * refImage = nullptr, * alphaPrecond = nullptr, * kerroin4 = nullptr, * lambdaFiltered = nullptr, * weights = nullptr, *listCoord = nullptr, * RDP_ref = nullptr;
	...
	std::vector<float> epsilon_mramla;
    std::vector<std::vector<float>> tauCP, tauCP2, LCP, LCP2;
    std::vector<float> alphaCP; // TODO vectorize these
