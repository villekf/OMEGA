...
void reconstructionAF(const float* z_det, const float* x, const F* Sin, const R* sc_ra, scalarStruct inputScalars,
		...
		if ((MethodList.MBSREM || MethodList.MRAMLA || MethodList.SPS || w_vec.precondTypeIm[6])) {
			...
			if ((w_vec.precondTypeIm[6] || MethodList.SPS) || (!inputScalars.CT && ((inputScalars.randoms_correction && !af::allTrue<bool>(rand > 0)) || inputScalars.randoms_correction == 0))) {
				...
				for (int ii = 0; ii <= inputScalars.nMultiVolumes; ii++) {
					...
					if (inputScalars.projector_type == 6) {
						forwardProjectionType6(E, w_vec, vec, inputScalars, inputScalars.nProjections, 0, ii);
					}
					else {
						af::sync();
				...
				if (w_vec.precondTypeIm[6] || MethodList.SPS) {
					...
					for (int ii = 0; ii <= inputScalars.nMultiVolumes; ii++) {
						if (inputScalars.projector_type == 6) {
							backprojectionType6(E, w_vec, vec, inputScalars, inputScalars.nProjections, 0, 0, 0, 0, iter0, ii);
						}
						else {
				...
				for (int ii = 0; ii <= inputScalars.nMultiVolumes; ii++) {
					if (inputScalars.listmode > 0 && inputScalars.computeSensImag) {
						...
						for (uint32_t subIter = 0; subIter < inputScalars.subsetsUsed; subIter++) {
							...
							oneInput.eval();
							if (inputScalars.projector_type == 6)
								backprojectionType6(oneInput, w_vec, vec, inputScalars, length[subIter], uu, subIter, 0, 0, 0, ii);
							else {
				...
				for (uint32_t ll = 0; ll < inputScalars.subsetsUsed; ll++) {
					...
					for (int ii = 0; ii <= inputScalars.nMultiVolumes; ii++) {
						...
						if (inputScalars.projector_type == 6) {
							...
							forwardProjectionType6(oneInput, w_vec, vec, inputScalars, length[ll], uu, ii);
							uu += length[ll];
						}
						else {
		...
		if (MethodList.RDP && w_vec.RDPLargeNeighbor && w_vec.RDP_anatomical) {
			...
			for (uint32_t ll = 0; ll < inputScalars.subsetsUsed; ll++) {
				...
				if (inputScalars.projector_type == 6) {
					...
					forwardProjectionType6(oneInput1, w_vec, vec, inputScalars, length[ll], uu, 0);
					uu += length[ll];
				}
				else {
				...
				if (inputScalars.projector_type == 6) {
					...
					forwardProjectionType6(oneInput2, w_vec, vec, inputScalars, length[ll], uu, 0);
					uu += length[ll];
				}
				else {
				...
				af::eval(oneInput1);
				if (inputScalars.projector_type == 6)
					backprojectionType6(oneInput1, w_vec, vec, inputScalars, length[ll], uu, ll, 0, 0, 0, 0);
				else {
		...
		for (uint32_t iter = iter0; iter < inputScalars.Niter; iter++) {
			...
			for (uint32_t osa_iter = inputScalars.osa_iter0; osa_iter < inputScalars.subsetsUsed; osa_iter++) {
				...
				// SPECT rotation-based projector
					...
					for (int ii = 0; ii <= inputScalars.nMultiVolumes; ii++)
						forwardProjectionType6(fProj, w_vec, vec, inputScalars, length[osa_iter], uu, ii);
					fProj.eval();
					fProj = af::flat(fProj);
					...
					if (status != 0)
						return;
					for (int ii = 0; ii <= inputScalars.nMultiVolumes; ii++)
						backprojectionType6(fProj, w_vec, vec, inputScalars, length[osa_iter], uu, osa_iter, iter, compute_norm_matrix, iter0, ii);
				}
				af::sync();
				if (DEBUG) {
					if (compute_norm_matrix > 1u) {
