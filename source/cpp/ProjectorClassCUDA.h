...
class ProjectorClass {
	...
	inline nvrtcResult createProgram(CUmodule& programFP, CUmodule& programBP,
		...
		if (MethodList.NLM || MethodList.MRP || MethodList.RDP || w_vec.precondTypeMeas[1] || w_vec.precondTypeIm[5]
			...
			if (MethodList.NLM || MethodList.ProxNLM) {
				...
				if (w_vec.NLM_MRP) {
					std::snprintf(buffer11, 30, "-DNLTYPE=%d", static_cast<int32_t>(2));
					optionsAux.push_back(buffer11);
				}
				else if (w_vec.NLTV) {
					std::snprintf(buffer11, 30, "-DNLTYPE=%d", static_cast<int32_t>(1));
					optionsAux.push_back(buffer11);
				}
				else if (w_vec.NLRD) {
					std::snprintf(buffer11, 30, "-DNLTYPE=%d", static_cast<int32_t>(3));
					optionsAux.push_back(buffer11);
				}
				else if (w_vec.NLLange) {
					std::snprintf(buffer11, 30, "-DNLTYPE=%d", static_cast<int32_t>(4));
					optionsAux.push_back(buffer11);
				}
				else if (w_vec.NLLangeFiltered) {
					std::snprintf(buffer11, 30, "-DNLTYPE=%d", static_cast<int32_t>(5));
					optionsAux.push_back(buffer11);
				}
				else if (w_vec.NLGGMRF) {
					std::snprintf(buffer11, 30, "-DNLTYPE=%d", static_cast<int32_t>(6));
					optionsAux.push_back(buffer11);
				}
				else {
					std::snprintf(buffer11, 30, "-DNLTYPE=%d", static_cast<int32_t>(0));
					optionsAux.push_back(buffer11);
				}
				if (w_vec.NLAdaptive)
	...
	inline CUresult createKernels(CUfunction& kernelFP, CUfunction& kernelBP, CUfunction& kernelNLM, CUfunction& kernelMed,
		...
		if (inputScalars.computeSensImag && inputScalars.listmode > 0) {
            ...
            CUDA_CHECK(status, "Failed to create sensitivity image kernels\n", status);
		}
		if (inputScalars.projector_type == 6) {
...
public:
	...
	CUdeviceptr* d_im, *d_rhs, *d_U, d_g, d_uref, *d_refIm, d_maskBPB, *d_RDPref;
    std::vector<CUdeviceptr> d_attenB;
	...
	inline int createAndWriteBuffers(const std::vector<int64_t>& length, const float* x, const float* z_det, const uint32_t* xy_index,
		...
		if (inputScalars.projector_type != 6) {
			...
			if ((!(inputScalars.CT || inputScalars.SPECT) && inputScalars.listmode == 0) || inputScalars.indexBased) {
				status = cuMemAlloc(&d_x[0][0], sizeof(float) * inputScalars.size_of_x);
				CUDA_CHECK(status, "\n", -1);
				memAlloc.xSteps++;
			}
			if (inputScalars.listmode > 0 && inputScalars.computeSensImag) {
            ...
            for (uint32_t timestep = 0; timestep < inputScalars.Nt; timestep++) {
                if (inputScalars.attenuation_correction && inputScalars.CTAttenuation) {
					if (inputScalars.size_atten > inputScalars.im_dim[0] || timestep == 0) {
						if (inputScalars.useBuffers)
							status = cuMemAlloc(&d_attenB[timestep], sizeof(float) * inputScalars.im_dim[0]);
						else {
							std::memset(&texDesc, 0, sizeof(texDesc));
							std::memset(&resDesc, 0, sizeof(resDesc));
							std::memset(&arr3DDesc, 0, sizeof(arr3DDesc));
			    ...
			    for (uint32_t kk = inputScalars.osa_iter0; kk < inputScalars.subsetsUsed; kk++) {
					if (inputScalars.CT || inputScalars.SPECT) {
						...
						status = cuMemAlloc(&d_x[timestep][kk], sizeof(float) * length[kk + timestep * inputScalars.subsets] * 6);
						CUDA_CHECK(status, "\n", -1);
						memAlloc.xSteps++;
					}
                    if ((inputScalars.CT || inputScalars.SPECT) && inputScalars.listmode != 1) {
                        ...
                        if (inputScalars.PET && inputScalars.listmode == 0) {
                            ...
                            status = cuMemAlloc(&d_z[timestep][kk], sizeof(float) * inputScalars.size_z);
                            memAlloc.zType = 0;
                            memAlloc.zSteps = kk;
                        }
						else {
                    ...
                    if (inputScalars.listmode > 0 && inputScalars.indexBased) {
						if (inputScalars.loadTOF || (kk == 0 && !inputScalars.loadTOF && timestep == 0)) {
                            status = cuMemAlloc(&d_trIndex[timestep][kk], sizeof(uint16_t) * length[kk + timestep * inputScalars.subsets] * 2);
                            CUDA_CHECK(status, "\n", -1);
                            status = cuMemAlloc(&d_axIndex[timestep][kk], sizeof(uint16_t) * length[kk + timestep * inputScalars.subsets] * 2);
                            CUDA_CHECK(status, "\n", -1);
                    ...
                    if (inputScalars.listmode > 0 && inputScalars.TOF) {
                        if (inputScalars.loadTOF || (kk == 0 && !inputScalars.loadTOF && timestep == 0)) {
                            status = cuMemAlloc(&d_TOFIndex[timestep][kk], sizeof(uint8_t) * length[kk + timestep * inputScalars.subsets]);
                            CUDA_CHECK(status, "\n", -1);
        ...
        CUDA_CHECK(status, "Buffer creation failed\n", -1);
		if (DEBUG || inputScalars.verbose >= 3) {
		...
		if (inputScalars.projector_type != 6) {
			...
			CUDA_CHECK(status, "\n", -1);
			if ((!(inputScalars.CT || inputScalars.SPECT) && inputScalars.listmode == 0) || inputScalars.indexBased) {
				status = cuMemcpyHtoD(d_x[0][0], x, sizeof(float) * inputScalars.size_of_x);
				CUDA_CHECK(status, "\n", -1);
			}
			if (inputScalars.listmode > 0 && inputScalars.computeSensImag) {
            ...
            for (uint32_t timestep = 0; timestep < inputScalars.Nt; timestep++) {
                if (inputScalars.attenuation_correction && inputScalars.CTAttenuation && inputScalars.useBuffers) {
					if (inputScalars.size_atten > inputScalars.im_dim[0] || timestep == 0) {
						status = cuMemcpyHtoD(d_attenB[timestep], &atten[inputScalars.im_dim[0] * timestep], sizeof(float) * inputScalars.im_dim[0]);
						CUDA_CHECK(status, "\n", -1);
					}
                }
			    for (uint32_t kk = inputScalars.osa_iter0; kk < inputScalars.subsetsUsed; kk++) {
                    if ((inputScalars.CT || inputScalars.SPECT) && inputScalars.listmode == 0) {
                        ...
                        if (inputScalars.pitch)
                            ...
                            kerroin = 1;
                        status = cuMemcpyHtoD(d_z[timestep][kk], &z_det[pituus[kk] * kerroin], sizeof(float) * length[kk] * kerroin);
                        CUDA_CHECK(status, "\n", -1);
                    }
                    else {
                        if (inputScalars.PET && inputScalars.listmode == 0) {
                    ...
                    if ((inputScalars.CT || inputScalars.SPECT) && inputScalars.listmode == 0) {
                        status = cuMemcpyHtoD(d_x[timestep][kk], &x[pituus[kk] * 6], sizeof(float) * length[kk] * 6);
                        CUDA_CHECK(status, "\n", -1);
                    }
						...
						if ((kk < inputScalars.TOFsubsets) || inputScalars.loadTOF || (!inputScalars.loadTOF && timestep == 0 && kk < inputScalars.TOFsubsets)) {
                            status = cuMemcpyHtoD(d_x[timestep][kk], &w_vec.listCoord[pituus[kk + timestep * inputScalars.subsets] * 6], sizeof(float) * length[kk + timestep * inputScalars.subsets] * 6);
                            CUDA_CHECK(status, "\n", -1);
                        }
                    }
                    if (inputScalars.scatter == 1U) {
                    ...
                    if (inputScalars.listmode > 0 && inputScalars.indexBased) {
                        if (inputScalars.loadTOF || (kk == 0 && !inputScalars.loadTOF && timestep == 0)) {
                            status = cuMemcpyHtoD(d_trIndex[timestep][kk], &w_vec.trIndex[pituus[kk + timestep * inputScalars.subsets] * 2], sizeof(uint16_t) * length[kk + timestep * inputScalars.subsets] * 2);
                            CUDA_CHECK(status, "\n", -1);
                            status = cuMemcpyHtoD(d_axIndex[timestep][kk], &w_vec.axIndex[pituus[kk + timestep * inputScalars.subsets] * 2], sizeof(uint16_t) * length[kk + timestep * inputScalars.subsets] * 2);
                            CUDA_CHECK(status, "\n", -1);
                        }
                    }
                    if (inputScalars.listmode > 0 && inputScalars.TOF) {
                        if (inputScalars.loadTOF || (kk == 0 && !inputScalars.loadTOF && timestep == 0)) {
                            status = cuMemcpyHtoD(d_TOFIndex[timestep][kk], &w_vec.TOFIndices[pituus[kk + timestep * inputScalars.subsets]], sizeof(uint8_t) * length[kk + timestep * inputScalars.subsets]);
                            CUDA_CHECK(status, "\n", -1);
                        }
                    }
                }
        ...
        CUDA_CHECK(status, "Buffer write failed\n", -1);
		if (DEBUG || inputScalars.verbose >= 3) {
	...
	inline int loadCoord(uint32_t currentSubset, scalarStruct& inputScalars, const int64_t length, const T* listCoord, const T* listCoordAx = nullptr, const uint8_t* TOFIndices = nullptr) {
        ...
        if (inputScalars.indexBased) {
            ...
            status = cuMemAlloc(&d_x[0][0], sizeof(float) * length * 6);
            CUDA_CHECK(status, "\n", -1);
            status = cuMemcpyHtoD(d_x[0][0], listCoord, sizeof(float) * length * 6);
            CUDA_CHECK(status, "\n", -1);
        }
        if (inputScalars.TOF) {
	...
	inline int forwardProjection(scalarStruct& inputScalars, Weighting& w_vec, uint32_t osa_iter, uint32_t timestep, const std::vector<int64_t>& length, uint64_t m_size, int ii = 0, const int uu = 0) {
        ...
        if (inputScalars.attenuation_correction && !inputScalars.CTAttenuation) {
			...
			if (inputScalars.size_atten > inputScalars.im_dim[0]) {
				...
				if (inputScalars.useBuffers)
					kTemp.emplace_back(&d_attenB[0]);
				else
					kTemp.emplace_back(&d_attenIm[0]);
			}
        }
		//}
		...
		if (inputScalars.FPType == 4) {
			...
			kTemp.emplace_back((void*)&length[osa_iter + timestep * inputScalars.subsets]);
	...
	inline int backwardProjection(scalarStruct& inputScalars, Weighting& w_vec, uint32_t osa_iter, uint32_t timestep, std::vector<int64_t>& length, uint64_t m_size, const bool compSens = false, int ii = 0, const int uu = 0) {
		...
		if (inputScalars.BPType == 1 || inputScalars.BPType == 2 || inputScalars.BPType == 3) {
			...
			if ((inputScalars.CT || inputScalars.PET || inputScalars.SPECT) && inputScalars.listmode == 0)
				kTemp.emplace_back(&length[indD]);
			if (compSens) {
	...
	inline int computeMRP(const scalarStruct& inputScalars, const uint64_t global_size[]) {
		...
		if (DEBUG || inputScalars.verbose >= 3)
			cuEventRecord(tEnd, CLCommandQueue[0]);
        CUDA_CHECK(status, "Failed to launch the Median filter kernel\n", -1);
		if (DEBUG || inputScalars.verbose >= 3) {
	...
	inline int ProxTVDiv(const scalarStruct& inputScalars) {
		...
		status = cuCtxSynchronize();
        CUDA_CHECK(status, "Queue finish failed before divergence kernel\n", -1);
		kArgs.emplace_back(&d_N[0]);
		...
		status = cuCtxSynchronize();
        CUDA_CHECK(status, "Queue finish failed after divergence kernel\n", -1);
		if (inputScalars.verbose >= 3)
	...
	inline int ProxTVGrad(const scalarStruct& inputScalars, float sigma2, const size_t vSize) {
		...
		CUDA_CHECK(status, "Failed to launch the Proximal TV gradient kernel\n", -1);
        if (DEBUG || inputScalars.verbose >= 3) {
			mexPrint("Proximal TV gradient kernel launched successfully\n");
		}

		status = cuCtxSynchronize();
        CUDA_CHECK(status, "Queue finish failed after gradient kernel\n", -1);
		if (inputScalars.verbose >= 3)
	...
	inline int ProxTGVSymmDeriv(const scalarStruct& inputScalars, float sigma2) {
        ...
        CUDA_CHECK(status, "Failed to launch the Proximal TGV symmetric derivative kernel\n", -1);
		if (DEBUG || inputScalars.verbose >= 3) {
			mexPrint("Proximal TGV symmetric derivative kernel launched successfully\n");
		}

		status = cuCtxSynchronize();
        CUDA_CHECK(status, "Queue finish failed after symmetric derivative kernel\n", -1);
		if (inputScalars.verbose >= 3)
	...
	inline int elementWiseComp(const bool mult, const uint64_t size[], bool D2 = false) {
        ...
        CUDA_CHECK(status, "Failed to launch the element-wise kernel\n", -1);

		status = cuCtxSynchronize();
        CUDA_CHECK(status, "Queue finish failed after element-wise kernel\n", -1);
		return 0;
	}
	...
	inline int PoissonUpdate(const scalarStruct& inputScalars, float lambda, float epps, float alpha, const int ii = 0) {
		...
		status = cuCtxSynchronize();
        CUDA_CHECK(status, "Queue finish failed after Poisson update kernel\n", -1);
		if (DEBUG || inputScalars.verbose >= 3) {
			cuEventSynchronize(tEnd);
			float milliseconds = 0;
			cuEventElapsedTime(&milliseconds, tStart, tEnd);
	...
	inline int PDHGUpdate(const scalarStruct& inputScalars, float epps, float theta, float tau, const int ii = 0) {
		...
		status = cuCtxSynchronize();
        CUDA_CHECK(status, "Queue finish failed after PDHG update kernel\n", -1);
		if (DEBUG || inputScalars.verbose >= 3) {
			cuEventSynchronize(tEnd);
			float milliseconds = 0;
			cuEventElapsedTime(&milliseconds, tStart, tEnd);
