...
inline int computeOSEstimates(AF_im_vectors& vec, Weighting& w_vec, const RecMethods& MethodList, const uint32_t iter,
	...
	if (!MethodList.BSREM && !MethodList.ROSEMMAP && !MethodList.POCS && !MethodList.SART && kk == 0) {
		...
		if (status != 0)
			return -1;
		if (DEBUG) {
			mexPrintBase("rhs_os[timestep]0 = %f\n", af::sum<float>(vec.rhs_os[timestep][0]));
			mexEval();
		}
	}
	...
	for (int ii = kk; ii <= inputScalars.nMultiVolumes; ii++) {
		...
		if (MethodList.OSLOSEM) {
			...
			status = SART(inputScalars, w_vec, MethodList, vec, proj, mData, g, length, pituus, osa_iter, iter , *Sens, vec.rhs_os[timestep][ii], w_vec.lambda[iter], ii);
			if (MethodList.POCS)
