...
typedef struct AF_im_vectors_ {
	af::array C_co, C_aco, C_osl;
	af::array wLSQR, fLSQR, pCP, uCP, rCGLS, fCGLS, qCPTV;
	af::array im_os, im_os_blurred, meanFP, meanBP;//rhs_os, 
	std::vector<af::array> rhs_os;
} AF_im_vectors;

// Struct for various parameters, mainly various weights and coefficients
typedef struct Weighting_ {
	af::array tr_offsets, weights_quad, weights_TV, weights_huber, fmh_weights, a_L, weighted_weights, UU, Amin, D, weights_RDP, preRef;
	...
	float epsilon_mramla = 1e8f, U, NLM_gauss = 1.f, h2 = 1.f, huber_delta = 0.f, ACOSEM_rhs = 0.f, h_ACOSEM_2 = 1.f, RDP_gamma, tauCP = 0.f,
		...
		phiLSQR = 0.f, gammaCGLS = 0.f, alphaCGLS = 0.f, nuIEM = 0.f, alphaCPTV = 0.f;
	uint32_t alku_fmh = 0u, mean_type = 0u, deblur_iterations = 0, powerIterations = 0, precondTypeIm = 0, precondTypeMeas = 0, derivType = 0;
	...
	bool med_no_norm = false, MBSREM_prepass = false, NLM_MRP = false, NLTV = false, NLM_anatomical = false, deconvolution = false, IEM = false;
...
typedef struct RecMethods_ {
	bool OSEM = false, MRAMLA = false, RAMLA = false, ROSEM = false, RBI = false, DRAMA = false, COSEM = false, ECOSEM = false,
		ACOSEM = false, LSQR = false, CGLS = false, CPLS = false, CPLSKL = false;
	...
	bool OSLOSEM = false, MAPOSEM = false, MBSREM = false, BSREM = false, ROSEMMAP = false, ROSEMOSL = false, RBIMAP = false, RBIOSL = false,
		PKMA = false, CPTV = false, CPTVKL = false;
	bool MAP = false;
	bool CUSTOM = false;
	bool initAlg = false;
...
typedef struct _CUDA_im_vectors {
	CUdeviceptr d_rhs_os, d_meanFP, d_meanBP;
	//cl::Image3D d_image_os, d_image_os_int;
