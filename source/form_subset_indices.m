function [options, lor_a, xy_index, z_index, LL, summa, pituus, varargout] = form_subset_indices(options, pituus, subsets, index, size_x, y, rings, fpbp, varargin)
%% FORM SUBSET INDICES
...
if options.precompute_lor
    ...
    if exist(lor_file, 'file') == 2 && exist('lor','var') ~= 1 && ~options.CT && ~options.SPECT
    ...
    elseif exist('lor','var') ~= 1 && ~options.CT && ~options.SPECT
        lor_pixel_count_prepass(options);
        if options.implementation == 1 || options.implementation == 4
            load(lor_file,'lor')
            if options.projector_type == 2 && options.implementation == 1
    ...
    if (exist('lor','var') ~= 1 || isempty(lor)) && (options.CT || options.SPECT)
    ...
    discard = lor > 0;
    if options.use_raw_data == false && length(discard) ~= options.TotSinos*options.Ndist*options.Nang
...
if options.listmode > 0 && options.subset_type < 8 && subsets > 1
    ...
    xy_index = uint32(0);
    z_index = uint16(0);
elseif options.listmode > 0 && (options.subset_type == 8 || options.subset_type == 9) && subsets > 1
    if isfield(options, 'z_det')
        options.z_det = options.z_det(index,:);
    else
        options.z = options.z(index,:);
...
if isempty(pituus) || sum(pituus) == 0
    pituus = [0; options.NSinos];
end
