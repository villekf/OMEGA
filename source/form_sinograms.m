...
if options.use_machine ~= 2
    ...
    elseif data_load
        ...
        if options.obtain_trues && options.use_machine == 0 && ~isempty(varargin{4})
            true_coincidences = initial_michelogram(options, varargin{4}, 'trues');
        end
        if options.store_scatter && options.use_machine == 0 && ~isempty(varargin{5})
            scattered_coincidences = initial_michelogram(options, varargin{5}, 'scatter');
        end
        if options.store_randoms && options.use_machine == 0 && ~isempty(varargin{6})
            random_coincidences = initial_michelogram(options, varargin{6}, 'randoms');
        end
