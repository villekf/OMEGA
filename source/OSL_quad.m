function x_quad = OSL_quad(options)
%OSL_quad One Step Late algorithm with quadratic prior reconstruction.
%   Implements the OSL quadratic prior reconstruction on input PET data.
%   See main_nongate.m for options-variables.
%
%   x_quad = OSL_quad(options) returns the OSL quadratic prior
...
for ii = 1 : options.Niter
    ...
    for kk = 1 : options.subsets
        ...
        quad_apu=(quad_apu./(full(sum(A,1)')+options.beta_quad_osl*med)).*((A'*(uu./(A*quad_apu+epps))));
        if options.Nz==1
            pz_pad_osl = padding(reshape(quad_apu,options.Nx,options.Ny),[options.Ndx options.Ndy]);
        else
            pz_pad_osl = padding(reshape(quad_apu,options.Nx,options.Ny,options.Nz),[options.Ndx options.Ndy options.Ndz]);
        end
        tElapsed = toc(tStart);
        disp(['OSL quadratic prior sub-iteration ' num2str(kk) ' took ' num2str(tElapsed) ' seconds'])
        disp(['OSL quadratic prior sub-iteration ' num2str(kk) ' finished'])
    end
    x_quad(:,ii+1) = quad_apu;
end
x_quad = reshape(x_quad,options.Nx,options.Ny,options.Nz, options.Niter + 1);
