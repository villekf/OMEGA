function [pz, varargout] = images_to_cell(im_vectors, llo, pz, options, rekot)
...
for kk = 1 : numel(varNonMAP)
    pz{loc(gg), llo} = im_vectors.(varNonMAP{kk});
    gg = gg + 1;
end
...
for ll = 1 : numel(varPrior)
    for kk = 1 : numel(varMAP)
        pz{loc(gg), llo} = im_vectors.([varPrior{ll} '_' varMAP{kk}]);
        gg = gg + 1;
    end
end
if nargout > 1
