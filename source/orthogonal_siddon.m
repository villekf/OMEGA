function [ lor, indices, alkiot, discard] = orthogonal_siddon( Ny, Nx, Nz, d, dz, by, bx, bz, z_det, x, y, yy, xx , NSinos, NSlices, vaimennus, index, pituus, attenuation_correction)
        ...
        for ii = 1 : Np
            ...
            if tx0 < ty0
                ...
                if tempj == Ny - 1 && xyz ~= 2
                    uu = 0;
                    for ky = 1 : Ny
                        d_ort = abs((yd - ys)*x_center(tempi) - (xd - xs)*y_center(tempj-uu) + xd*ys - yd*xs)/sqrt((yd - ys)^2 + (xd - xs)^2);
                        if d_ort > cr_p
                            break
                        end
                        hpk = hpk + 1;
                        idx(hpk) = (tempj - uu)*Ny+tempi+tempk;
                        uu = uu + 1;
                        templ_ijk(hpk) = d_ort;
                        if tempj - uu < 1
                            break
                ...
                elseif xyz ~= 2
                    ...
                    if xyz ~= 2
                        for ky = 1 : Ny
                            d_ort = abs((yd - ys)*x_center(tempi) - (xd - xs)*y_center(tempj-uu + 1) + xd*ys - yd*xs)/sqrt((yd - ys)^2 + (xd - xs)^2);
                            if d_ort > cr_p
                                break
                            end
                            hpk = hpk + 1;
                            idx(hpk) = (tempj - uu + 1)*Ny+tempi+tempk;
                            uu = uu + 1;
                            templ_ijk(hpk) = d_ort;
                            if tempj - uu < 0
                                break
