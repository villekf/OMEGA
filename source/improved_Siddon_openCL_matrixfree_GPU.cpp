...
void reconstruction(const size_t koko, const uint16_t* lor1, const float* z_det, const float* x, const float* y, const float* Sin, const int Nx, const int Ny, 
	...
	const float zmax, const float xxv, const float yyv, const float xxa, const float yya, const float NSlices, const uint32_t* pituus, const size_t koko_l, 
	const uint32_t* xy_index, const uint32_t* z_index, const int size_x, const int TotSinos, float* mlem, float* osem, const bool verbose, 
	...
	for (int iter = 0; iter < Niter; iter++) {
		...
		for (int osa_iter = 0; osa_iter < subsets; osa_iter++) {
			...
			//cl_mem d_rhs = clCreateBuffer(af_context, CL_MEM_READ_WRITE, im_dim, NULL, &status);
			//cl_mem d_Summ = clCreateBuffer(af_context, CL_MEM_READ_WRITE, im_dim, NULL, &status);
...
{
	if (nrhs != 37)
		mexErrMsgTxt("Invalid number of input arguments.  There must be exactly 37.");
	...
	uint32_t *pituus = (uint32_t*)mxGetData(prhs[23]);
	...
	uint32_t* xy_index = (uint32_t*)mxGetData(prhs[32]);
	size_t koko = mxGetNumberOfElements(prhs[32]);

	uint32_t* z_index = (uint32_t*)mxGetData(prhs[33]);

	int* maksimi = (int*)mxGetData(prhs[34]);

	bool verbose = (bool)mxGetScalar(prhs[35]);

	int device = (int)mxGetScalar(prhs[36]);
	...
	reconstruction(koko, lor1, z_det, x, y, Sin, Nx, Ny, Nz, Niter, x0, d, dz, bxf, byf, bzf, bxb, byb, bzb, bx, by, bz, maxxx, maxyy, minxx, minyy, zmax, xxv, 
		yyv, xxa, yya, NSlices, pituus, koko_l, xy_index, z_index, size_x, TotSinos, ele_ml, ele_os, verbose, attenuation_correction, atten, size_atten, 
