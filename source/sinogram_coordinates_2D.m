function [varargout] = sinogram_coordinates_2D(options)
...
if mashing > 1
    n = 2;
    xp = arrayfun(@(i) mean(xp(i:i+n-1)),1:n:det_w_pseudo-n+1)';
    yp = arrayfun(@(i) mean(yp(i:i+n-1)),1:n:det_w_pseudo-n+1)';
