function x_mrp = OSL_MRP(options)
%OSL_MRP One-Step Late algorithm with Median Root Prior reconstruction.
%   Implements the OSL_MRP reconstruction on input PET data.
%   See main_nongate.m for options-variables.
%
%   x_mrp = OSL_MRP(options) returns the OSL_MRP reconstructions for all
...
for ii = 1 : options.Niter
    ...
    for kk = 1 : options.subsets
        ...
        if options.Nz==1
            ...
            med = medfilt3(reshape(mrp_apu,options.Nx,options.Ny,options.Nz), [options.medx options.medy options.medz]);
        end
        med = med(:);
        mrp_apu=((mrp_apu)./(full(sum(A,1)')+options.beta_mrp_osl*(mrp_apu-med)./(med+epps))).*((A'*(uu./(A*mrp_apu+epps))));
        tElapsed = toc(tStart);
