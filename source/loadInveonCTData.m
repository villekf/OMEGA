...
options.ySize = str2double(hdr{find(strcmp(hdr,'x_dimension')) + 1});
options.xSize = str2double(hdr{find(strcmp(hdr,'y_dimension')) + 1});
...
% binningTr = croppedTrPixels / xSize;
% binningAx = croppedAxPixels / xSize;
...
fclose(fid);
C = projData((numel(projData) - options.xSize*options.ySize*options.zSize*options.nBed - options.xSize*options.ySize*2) + 1:...
    numel(projData) - options.xSize*options.ySize*options.zSize*options.nBed);
dark = C(1:end/2);
light = C(end/2+1:end) - dark;
dark = reshape(dark, options.ySize, options.xSize);
light = reshape(light, options.ySize, options.xSize);
projData = projData((numel(projData) - options.xSize*options.ySize*options.zSize*options.nBed) + 1:end);
projData = reshape(projData,options.ySize,options.xSize,options.zSize,options.nBed);
if numel(options.use_N_positions) ~= options.nBed
...
if options.binning > 1
    ...
    projData = B;
    options.ySize = size(projData,1);
    options.xSize = size(projData,2);
end
