...
cl_int createAndWriteBuffers(std::vector<cl::Buffer>& d_x, std::vector<cl::Buffer>& d_z, cl::Buffer& d_x_mlem, cl::Buffer& d_z_mlem,
	...
	cl::Image2D& d_maskBP, const uint8_t listmode = 0, const bool CT = false);
...
void MRAMLA_prepass(const uint32_t subsets, const uint32_t im_dim, const int64_t* pituus, const std::vector<cl::Buffer>& lor, const std::vector<cl::Buffer>& zindex,
	...
	const size_t koko, const bool randoms_correction, const size_t local_size[], const uint64_t* randSize = nullptr, const uint32_t Nt = 1U, const bool CT = false);
...
void forwardProjection(const RecMethods& MethodList, af::array& y, af::array& input, const size_t length, const scalarStruct& inputScalars, 
