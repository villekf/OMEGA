...
% Note: No example data is available for Biograph mCT
...
%%% Axial FOV (mm)
options.axial_fov = 218;

%%% Number of pseudo rings between physical rings (use 0 or [] if none)
% NOTE: mCT has 3 pseudo rings.
options.pseudot = 3;
% Alternatively, you can have no pseudo rings but instead have only the
% gaps between the rings. In such a case, put options.pseudot = 0 and
% uncomment the below line.
% options.ringGaps = [13;26;39];
...
% NOTE: mCT has one pseudo detector on each block
options.det_w_pseudo = options.blocks_per_ring*(options.cryst_per_block + 1);

%%% Number of crystal rings
options.rings = options.linear_multip * options.cryst_per_block + sum(options.pseudot);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%% BIOGRAPH mCT DATA %%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Use Biograph mCT data
% 0/false = Don't use Biograph data (use GATE data instead)
% 1 = Use 64-bit list-mode data
% 2 = Use sinogram data (.s/.ptd)
% 3 = Use 32-bit list-mode data
options.use_machine = 3;
...
% NOTE: On mCT, the sinogram has mashing of 2 by default
options.Nang = options.det_w_pseudo / 4;
...
% Either MATLAB's built-in fillmissing or fillmissing2, or inpaint_nans
% from file exchange.
% For inpaint_nans see: 
% https://se.mathworks.com/matlabcentral/fileexchange/4551-inpaint_nans
% See wiki for more details.
% NOTE: GNU Octave does not support fillmissing or fillmissing2.
options.gap_filling_method = 'fillmissing2';
...
options.interpolation_method_inpaint = 4;
...
% function.
options.randoms_smoothing = true;
...
% If set to true, performs normalization correction to the scatter
% coincidences data during sinogram formation or before reconstruction. If
% set to false, the scatter data is subtracted from the sinogram before
% normalization (and when options.corrections_during_reconstruction =
% false). 
...
% Specify the folder for the attenuation (DICOM) images. If left empty and
% attenuation correction has been selected above, prompts the user to
% select the folder.
...
% If set to true, normalization correction is applied to either sinogram
...
% true. Alternatively you can use user-made normalization data (see below).
...
% future sessions.
options.name = 'mCT_examination_test';
...
if ispc % Windows
    options.fpath = 'C:\path\to\input\data';
else % Unix/MAC
    options.fpath = '/path/to/input/data/';
end
...
% TVtype, e.g. for type 1 it is the edge threshold parameter. See the doc
% for more details:
% https://omega-doc.readthedocs.io/en/latest/algorithms.html#tv
options.T = 0.5;
...
if options.only_reconstructions == false || options.precompute_all && ~options.only_reconstructions
    if options.use_machine == 0
        % GATE data
        options.SinM = load_data(options);
        % Save the delayed coincidences, trues, scatter and randoms to
        % workspace (GATE only):
        % [options.SinM, SinDelayed, SinTrues, SinScatter, SinRandom] = load_data(options);
        % Save the coordinates of the detected coincidences to work space (GATE only):
        % [~, ~, ~, ~, ~, options.x] = load_data(options);
    elseif options.use_machine == 3 || options.use_machine == 2
        % 32-bit listmode or (uncompressed) sinogram data
        options.SinM = load_data_mCT(options);
    else
        % 64-bit list-mode data or Sinogram data
        % options.SinM = load_data_mCT(options);
        % List-mode reconstruction:
        [~, ~, options.x] = load_data_mCT(options);
        options.SinM = ones(numel(options.x) / 6, 1, 'single');
    end
end

%% Attenuation correction

if options.attenuation_correction && ~options.only_reconstructions && isempty(options.attenuation_datafile) && options.CT_attenuation
    options.attenuation_factors = create_atten_matrix_CT(options, [], options.name);
end
