%% MATLAB codes for CT reconstruction using GATE projection data
% This example script uses the "classic" example from GateContrib
% (https://github.com/OpenGATE/GateContrib/tree/master/imaging/CT/classic)
...
% matrix-free MLEM, OSEM and Chambolle-Pock with TV as well as matrix
% reconstruction with MLEM. 
...
% dimensions
options.xSize = 100/options.binning;
...
% NOTE: if you use binning, this value has to use the final binned
% dimensions
options.ySize = 100/options.binning;
...
if ~options.only_reconstructions || ~isfield(options,'SinM')
    % Set the second last false to true to save the projection images to
    % mat file with the name specified by options.name
    options.SinM = loadProjectionData('single',[options.xSize options.ySize options.nProjections], options.binning, [], true, false, options.name);
...
% Any variable named f_osem will be automatically visualized by
% visualize_pet.m as long as the pz-variable does not exist
f_osem = ff;
...
% NOTE: The CP code is based on: https://doi.org/10.5281/zenodo.4543225
...
% tau = 1/powerMethod(A, 10, true);
tau = 0.009096830095107;
alpha = 0.01;
Niter = 50;
sigma = tau;
sigma2 = sigma;
...
%             + 4 .* sigma .* raw_SinM));
        ...
        if sDims >= 3
            ...
            q = qHelp./max(1,sqrt(qHelp2 + circshift(qHelp2,N))/alpha);
        end
...
%% Matrix example using OSEM
% This example uses subsets
...
% [x,y,z] = CTDetectorCoordinatesFull(-options.angles,options.sourceToDetector,options.sourceToCRot,options.dPitch,options.xSize,...
%     options.ySize);
% C = forwardBackwardProjectCT(x,y,z,options.Nx,options.Ny,options.Nz,options.FOVa_x,options.axial_fov, options.subsets, options.implementation, options.use_device);
% C = forwardBackwardProjectCT(options.angles, options.ySize, options.xSize,options.Nx,options.Ny,options.Nz,options.FOVa_x,options.axial_fov, ...
%     options.dPitch, options.sourceToDetector, options.sourceToCRot, 0, 0, options.subsets, options.implementation, options.use_device);
raw_SinM = double(options.SinM(C.index));

f = options.x0(:);
% If subsets are used
% raw_SinM = double(raw_SinM(A.index));
% Form the full system matrix with this:
% NOTE: This is the TRANSPOSE of the system matrix
% B = formMatrix(C);
for iter = 1 : 2
    for osa_iter = 1 : options.subsets
