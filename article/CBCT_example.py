# -*- coding: utf-8 -*-
def CBCTExample(path, iType, deviceNum = 0, largeDim = False):
    """
    ...
    the left, type 1 middle, and type 2 to right. You need to provide the 
	full path to the input data
    Planmeca_VisoG7_100kV_80mAs_500proj_kneePhantom.mat!
    The device number can be set after the type, e.g. CBCT_example(path, type, deviceNum = 1)
    """
    import numpy as np
    ...
    ### Number of subsets
    if iType == 0 or iType == 1 or iType == 3 or largeDim:
        options.subsets = 20
    else:
        options.subsets = 1
