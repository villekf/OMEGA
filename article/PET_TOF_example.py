def PETTOFExample(path, iType, deviceNum = 0):
    """
    Function used to call each example individually
    If you want a standalone script for the examples, see the PET_exampleX.py
...
# always set as false.
    ...
    if options.useIndexBasedReconstruction:
        options.SinM = np.ones(options.trIndex.shape[1], dtype=np.uint16)
